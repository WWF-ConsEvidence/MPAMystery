# 
# code:  TNTC 2Yr Impact Summary Plots
# 
...
# 
# ---- inputs ----
#  1) Source TNTC.ImpactSumm.2yr.SigTests.R 
...
# 
# ---- code sections ----
#  1) Source TNTC 2Yr Impact Summary Statistical Code
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#
# ---- SECTION 1: Source TNTC 2Yr Impact Summary Statistical Code ----
#
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# 


source('2_Social/ImpactSummaries/BHS/SignificanceTestCodes/TNTC.ImpactSumm.2yr.SigTests.R')
...
TNTC.MPAimpact.summ.fs.st <- ggplot(data=TNTC.2yr.status.trend,
                                    aes(Year,
                                        FS)) +
...
TNTC.MPAimpact.summ.ma.st <- ggplot(data=TNTC.2yr.status.trend,
                                    aes(Year,
                                        MA)) +
  geom_bar(aes(fill=Treatment),
           width=0.5,
...
TNTC.MPAimpact.summ.pa.st <- ggplot(data=TNTC.2yr.status.trend,
                                    aes(Year,
                                        PA)) +
  geom_bar(aes(fill=Treatment),
           width=0.5,
...
TNTC.MPAimpact.summ.mt.st <- ggplot(data=TNTC.2yr.status.trend,
                                    aes(Year,
                                        MT)) +
  geom_bar(aes(fill=Treatment),
           width=0.5,
...
TNTC.MPAimpact.summ.se.st <- ggplot(data=TNTC.2yr.status.trend,
                                    aes(Year,
                                        SE)) +
  geom_bar(aes(fill=Treatment),
           width=0.5,
...
TNTC.MPAimpact.summ.pa.i <- ggplot(data=TNTC.2yr.impacts,
  ...
  geom_errorbar(aes(ymin=PA-PAErr,
                ...
                position="dodge",
                show.legend=F) +
  annotate("text",y=TNTC.sig.pos["PA",],x=0.65,
...
snapshot.TNTC.MPAimpact.summ <- ggplot(data=TNTC.2yr.std.impacts,
  ...
  labs(x="Human Well-Being Domain\n ",y="\n MPA Impact",
       title="Social Impacts of Teluk Cenderawasih National Park") +
