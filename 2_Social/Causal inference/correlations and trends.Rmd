...
p.foodsec <- t.plot(foodsec,'MonitoringYear','FSIndex') +
  facet_grid(.~MPAName) 
p.foodsec
...
p.priocc <-  ggplot(prim.occ.data, aes(x=MonitoringYear, y=FSIndex, group=MPAName,label=n)) + 
...
my_pointplot_ci(prim.occ.data,'MonitoringYear','FSIndex','MPAName') +
  ylab("FSIndex") +
  xlab("MonitoringYear") +
  annotate("rect", xmin = 0, xmax = 4, ymin = 0, ymax = 1.5,alpha = .1,fill='red') +
  annotate("rect", xmin = 0, xmax = 4, ymin = 1.5, ymax = 4,alpha = .1,fill='yellow') +
...
p.primincD <-  ggplot(prim.inc, aes(x=MonitoringYear, y=FSIndex, group=MPAName,label=n,)) + 
              geom_errorbar(aes(ymin=FSIndex- se, ymax=FSIndex+ se), width=0, position = pd ) +
              geom_line(position =pd) +
              geom_point(size=2, position =pd)+ theme_bw() +
              geom_text(size = 3, position = position_stack(vjust = 0.5)) +
...
prim.inc1 <- HHData %>% 
 ...
 filter(!is.na(PercentIncFishClean) & PercentIncFishClean!=0) %>% 
 mutate(fish.inc1=ifelse(PercentIncFishClean>3,'majority','minority')) %>% 
summarySE(measurevar="FSIndex", groupvars=c('MPAName','fish.inc1',"MonitoringYear"),na.rm = T) 
