...
MPA.TechReport.SigTest.Data <- HHData %>%
  select("HouseholdID", "MPAID", "SettlementName", "Treatment", "InterviewYear", 
         "MonitoringYear","MTIndex","MAIndex", "FSIndex", "PAIndex",
         "SERate","TimeMarket","DaysUnwell")

# - "Trend" dataset

Alor.Trend.Data <- MPA.TechReport.SigTest.Data %>%
  filter(MPAID==15 & SettlementName != "Alila TImur")
...
Alor.TechReport.MPAHouseholdData <- 
  MPA.TechReport.SigTest.Data[MPA.TechReport.SigTest.Data$Treatment==1 &
                                MPA.TechReport.SigTest.Data$MPAID==15 &
                                MPA.TechReport.SigTest.Data$MonitoringYear=="3 Year Post" &
                                MPA.TechReport.SigTest.Data$SettlementName!="Alila TImur",]
...
Alor.TechReport.MPAvControl <- 
  MPA.TechReport.SigTest.Data[MPA.TechReport.SigTest.Data$MPAID==15,]


# - "Settlement Means" dataset
Alor.TechReport.Status.SettlementMeans<- Techreport.Status.BySett %>% 
  filter(MonitoringYear=="3 Year Post" & MPAID==15 & Treatment==1 & SettlementID > 116)
#UPDATE AND ADD BACK IN ALILA TIMUR AFTER UPDATED DATA IS IN
...
colnames(Alor.TechReport.Status.SettlementMeans) <- c(colnames(Alor.TechReport.Status.SettlementMeans)[1:52],"DaysUnwell","UnwellErr",
                                                        "TimeMarket","TimeMarketErr",colnames(Alor.TechReport.Status.SettlementMeans)[57:65],"MAMean",
                                                      colnames(Alor.TechReport.Status.SettlementMeans)[67:69],
                                                        "FSIndex","FSErr","MAIndex","MAErr","PAIndex","PAErr", "MTIndex","MTErr",
                                                        "SERate","SEErr")



Alor.FreqTables <- HHData %>%
  filter(MPAID==15 & Treatment==1) %>%
...
even.number.setts.function.Alor <- 
  mapply(a=Alor.TechReport.Status.SettlementMeans[,c("FSIndex","MAIndex","MTIndex","PAIndex","SERate","TimeMarket","DaysUnwell")],
         b=Alor.TechReport.MPAHouseholdData[,c("FSIndex","MAIndex","MTIndex","PAIndex","SERate","TimeMarket","DaysUnwell")],
         function(a,b){
...
median.setts.Alor <- 
  mapply(i=Alor.TechReport.Status.SettlementMeans[,c("FSIndex","MAIndex","MTIndex","PAIndex","SERate","TimeMarket","DaysUnwell")],
         j=names(even.number.setts.function.Alor),
         function(i,j){
           med <- median(i,na.rm=T)
           med.setts <- factor(ifelse(length(sett.names.Alor)%%2!=0,
                                      as.character(Alor.TechReport.Status.SettlementMeans$SettlementName[which(i==med)]),
...
dist.Alor.DaysUnwell <- 
  ggplot(Alor.TechReport.MPAHouseholdData) +
  geom_histogram(aes(x=Unwell,y=..density..),
                 bins=20,fill="#5971C7",colour="#262F52",na.rm=T) +
  stat_function(fun=dnorm,args=list(mean=mean(Alor.TechReport.MPAHouseholdData$Unwell),
                                    sd=sd(Alor.TechReport.MPAHouseholdData$Unwell)),
                colour="#262F52",size=1,na.rm=T) +
  labs(x="Days Unwell\n(per household)",
       y="Density",
...
qqnorm(Alor.TechReport.MPAHouseholdData$Unwell)
qqline(Alor.TechReport.MPAHouseholdData$Unwell,col="green")
                      ...
                      results <- 
                        list(cbind.data.frame(SettlementName=c(as.character(sett.names.Alor[which(sett.names.Alor!=median.setts.Alor[a])]),
                                              ...
                                              rbind.data.frame(t(data.frame(mapply(i=sett.names.Alor[which(sett.names.Alor!=median.setts.Alor[a])],
                                                                                   function(i){
                                                                                     var <- 
                                                                                       unlist(Alor.TechReport.MPAHouseholdData[Alor.TechReport.MPAHouseholdData$SettlementName==i |
...
sigvals.Sett.Alor <- 
  cbind.data.frame(non.parametric.test.settlements.Alor[order(non.parametric.test.settlements.Alor$"FSIndex.SettlementName"),
                   ...
                   non.parametric.test.settlements.Alor[order(non.parametric.test.settlements.Alor$"TimeMarket.SettlementName"),
...
colnames(sigvals.Sett.Alor) <- c("SettlementName","FS.pval","MA.pval","PA.pval","MT.pval","SE.pval","TimeMarket.pval","Unwell.pval")
...
non.parametric.test.MPAvControl.Alor <-
  data.frame(mapply(a=c("FSIndex","MAIndex","MTIndex","PAIndex","SERate","TimeMarket","DaysUnwell"),
                    function(a){
                      var <- unlist(Alor.TechReport.MPAvControl[,a])
...
sigvals.Alor[,c("FS.pval", "MA.pval", "MT.pval" , "PA.pval", "SE.pval", "TimeMarket.pval",
                  "Unwell.pval")] <- unlist(sigvals.Alor[,c("FS.pval", "MA.pval", "MT.pval", "PA.pval","SE.pval","TimeMarket.pval",
                                                              "Unwell.pval")])
# ---- 4.3 Define function for trend data significance ---- 
trend.non.parametric.test.byMPA.Alor <- 
  data.frame(mapply(i=Alor.Trend.Data[,c("FSIndex","MAIndex","MTIndex","PAIndex","SERate","TimeMarket","DaysUnwell")],
                    function(i){
...
trend.non.parametric.test.byControl.Alor <- 
  data.frame(mapply(i=Alor.Trend.Data[Alor.Trend.Data$Treatment==0,c("FSIndex","MAIndex","MTIndex","PAIndex","SERate","TimeMarket","DaysUnwell")],
                    function(i){
                      MannKendall(c(i[Alor.Trend.Data$InterviewYear==unique(Alor.Trend.Data$InterviewYear)[1]],
                                    i[Alor.Trend.Data$InterviewYear==unique(Alor.Trend.Data$InterviewYear)[2]]))
                    }))

colnames(trend.non.parametric.test.byMPA.Alor) <- colnames(sigvals.Alor[2:8])
colnames(trend.non.parametric.test.byControl.Alor) <- colnames(sigvals.Alor[2:8])

trend.sigvals.Alor <- 
  cbind.data.frame(trend.non.parametric.test.byMPA.Alor["sl",1],MonitoringYear="p.value",trend.non.parametric.test.byMPA.Alor["sl",1],NA,trend.non.parametric.test.byMPA.Alor["sl",2],
                   ...
                   NA,trend.non.parametric.test.byMPA.Alor["sl",6],NA,trend.non.parametric.test.byMPA.Alor["sl",7],NA)

colnames(trend.sigvals.Alor) <- c("SettlementName","MonitoringYear","FSMean","FSErr","MAMean","MAErr","PAMean","PAErr","MTMean","MTErr","SEMean","SEErr",
                                    "TimeMarketMean","TimeMarketErr","UnwellMean","UnwellErr")

trend.sigvals.Alor <- unlist(trend.sigvals.Alor)

control.sigvals.Alor <- 
  cbind.data.frame(trend.non.parametric.test.byControl.Alor["sl",1],MonitoringYear="p.value",trend.non.parametric.test.byControl.Alor["sl",1],NA,trend.non.parametric.test.byControl.Alor["sl",2],
                   NA,trend.non.parametric.test.byControl.Alor["sl",3],NA,trend.non.parametric.test.byControl.Alor["sl",4],NA,trend.non.parametric.test.byControl.Alor["sl",5],
                   NA,trend.non.parametric.test.byControl.Alor["sl",6],NA,trend.non.parametric.test.byControl.Alor["sl",7],NA)

colnames(control.sigvals.Alor) <- c("SettlementName","MonitoringYear","FSMean","FSErr","MAMean","MAErr","PAMean","PAErr","MTMean","MTErr","SEMean","SEErr",
                                      "TimeMarketMean","TimeMarketErr","UnwellMean","UnwellErr")

control.sigvals.Alor <- unlist(control.sigvals.Alor)



# ---- 4.4 Create function that will output TREND significance values for non-parametric variables, BY SETTLEMENT ----
#          (for annex plots)
Alor.Trend.Data <- as.data.frame(Alor.Trend.Data)
...
colnames(trend.non.parametric.test.bySett.Alor) <- colnames(sigvals.Alor[1:8])
...
annex.sigvals.Alor <- 
  rbind.data.frame(cbind.data.frame(SettlementName="MPA\n Settlements",trend.non.parametric.test.byMPA.Alor["sl",]),
...
annex.sigvals.Alor[2:8] <- unlist(annex.sigvals.Alor[2:8])
...
colnames(propdata.trend.test.Alor) <- c("Primary occupation (% households)","Frequency of fishing (% households)","Frequency of selling at least some catch (% households)",
