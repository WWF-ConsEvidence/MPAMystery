...
# ---- 2.1 ALOR: Rename and add new IDs----

Alor_FGD_Habitats <- Alor_FGD_Habitats %>%
  rename(c("FGDID"="FGDCode")) %>%
...
Alor_FGD_MPA <- Alor_FGD_MPA %>%
  rename(c("TraditionalGov"="TraditionalGovernance")) %>%
  .[order(.$FGDID),] %>%
...
Alor_FGD_Stakeholders <- Alor_FGD_Stakeholders %>%
  rename(c("Stakeh0lderID"="StakeholderID","FGDID"="FGDCode","C0untryC0de"="CountryCode","SiteC0de"="SiteCode",
...
Alor_KII_MPA <- Alor_KII_MPA %>%
  rename(c("KII_Field_Code"="KIICode","INTERVIEW.MONTH"="INTERVIEW MONTH","INTERVIEW.YEAR"="INTERVIEW YEAR")) %>%
  .[order(.$KII_ID),] %>%
...
Alor_KII_SppRules <- Alor_KII_SppRules %>%
  rename(c("KII_ID"="KIICode","SpeciesLFamily"="Family","SpeciesLGenus"="Genus",
           "SpeciesLSpecies"="Species")) %>%
  .[order(.$SPPRulesID,.$KIICode),] %>%
  mutate(KII_order=NA, SppSpecificRule=NA,
...
# ---- 2.2 FLOTIM: Rename and add new IDs----
...
Flotim_FGD_MPA <- Flotim_FGD_MPA %>%
  rename(c("TraditionalGov"="TraditionalGovernance","FGroundTime..minute."="FGroundTime",
...
Flotim_FGD_Stakeholders <- Flotim_FGD_Stakeholders %>%
  rename(c("Stakeh0lderID"="StakeholderID","FGDID"="FGDCode","CountryC0de"="CountryCode","SiteC0de"="SiteCode",
           "SettlementC0de"="SettlementCode")) %>%
  .[order(.$StakeholderID,.$FGDCode),] %>%
