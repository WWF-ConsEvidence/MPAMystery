...
setwd("C:/Users/david/Dropbox/data/analysis/IndSocImpacts/MPAMystery")
...
for (i in 1:ncol(select(WELLBEING,InterviewYear:BestIncomeUnits))){
  
  print(paste0(names(WELLBEING)[i]," (",class(WELLBEING[,i]),")"))
  
  if(length(unique(WELLBEING[,i]))>20 & class(WELLBEING[,i])!="character"){
    print(summary(WELLBEING[,i]))
  }
  
  else{
    if (length(unique(WELLBEING[,i]))<20){
...
for (i in 1:ncol(select(DEMOGRAPHIC,IndividualName:IndividualLostDays))){
  
  print(paste0(names(DEMOGRAPHIC)[i]," (",class(DEMOGRAPHIC[,i]),")"))
  
  if(length(unique(DEMOGRAPHIC[,i]))>20 & class(DEMOGRAPHIC[,i])!="character"){
    print(summary(DEMOGRAPHIC[,i]))
  }
  
  else{
    if (length(unique(DEMOGRAPHIC[,i]))<20){
