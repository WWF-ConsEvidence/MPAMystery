# 
# code:  Source social data from database for all social MPA Mystery analysis
# 
...
# 
# author: Matheus De Nardo, matheus.denardo@wwf.org
...
# ---- 1.2 Create connection with database ----

pg = dbDriver("PostgreSQL")

# if connected to wwinternal.org ethernet, use:
con = dbConnect(pg, user="", password="",
                host="WWFUS-ArcgisDev", port=5432, dbname="mpasocial")

# if NOT connected to wwinternal.org ethernet, use:
# con = dbConnect(pg, user="", password="",
...
# ---- 2.1 Call, clean, & post-code HH_tbl_WELLBEING to create IndDemos for analysis ----
...
# ---- 2.2 Call, clean, & post-code HH_tbl_DEMOGRAPHIC to create IndDemos for analysis ----
IndDemos <- 
  tbl(con, "HH_tbl_DEMOGRAPHIC") %>%
...
# ---- 2.3 Call, clean, & post-code HH_tbl_ORGANIZATION to create Organization for analysis ----

Organization <- 
  tbl(con, "HH_tbl_ORGANIZATION") %>%
  dplyr::transmute(HouseholdID = HouseholdID,
...
# ---- 2.4 Add seascape column to Settlements data frame for analysis ----

Settlements <- 
  tbl(con,"HH_tbl_SETTLEMENT") %>%
