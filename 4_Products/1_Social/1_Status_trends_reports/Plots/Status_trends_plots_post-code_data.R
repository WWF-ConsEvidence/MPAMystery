...
ThreatType <-
  import('x_Flat_data_files/1_Social/Outputs/Status_trends_analysis/SulawesiTenggara_20200415/SulawesiTenggara_LTHREAT-post coding.xlsx') %>%
...
MPA.level.ThreatType <- 
  ...
  mutate(SettlementID=NA,
         SettlementName=ifelse(Treatment==1,MPA.name$MPAName,"Control Settlements")) %>%
  group_by(MPAID,SettlementID,SettlementName,Treatment) %>%
  summarise(UnsustainableFish=(length(HouseholdID[MainThreat=="UnsustainableFishing" & !is.na(MainThreat)])/length(HouseholdID[!is.na(MainThreat)]))*100,
            Pollution=(length(HouseholdID[MainThreat=="Pollution" & !is.na(MainThreat)])/length(HouseholdID[!is.na(MainThreat)]))*100,
            NaturalPhenomenon=(length(HouseholdID[MainThreat=="Natural phenomenon" & !is.na(MainThreat)])/length(HouseholdID[!is.na(MainThreat)]))*100,
            Tourism=(length(HouseholdID[MainThreat=="Tourism & coastal development" & !is.na(MainThreat)])/length(HouseholdID[!is.na(MainThreat)]))*100,
...
MPA.level.PostCodeData <-
  ...
  filter(MPAID==20 & IndividualAge>=18 & ed.level<=5) %>%
...
Sett.level.PostCodeData <-
  ...
  filter(MPAID==20 & IndividualAge>=18 & ed.level<=5 & Treatment==1) %>%
  filter(InterviewYear==max(InterviewYear)) %>%
  group_by(SettlementID,SettlementName) %>%
  summarise(HHHEducNone=(length(DemographicID[ed.level==0 & !is.na(ed.level) & RelationHHH==0])/length(DemographicID[!is.na(ed.level) & RelationHHH==0]))*100,
            HHHEducPre=(length(DemographicID[ed.level==1 & !is.na(ed.level) & RelationHHH==0])/length(DemographicID[!is.na(ed.level) & RelationHHH==0]))*100,
...
FigureFileName.english <- 'x_Flat_data_files/1_Social/Outputs/Status_trends_analysis/SulawesiTenggara_20200415/Figures--produced_20200415'

FigureFileName.bahasa <- 'x_Flat_data_files/1_Social/Outputs/Status_trends_analysis/SulawesiTenggara_20200415/Bahasa_Figures--produced_20200415'
...
export(ThreatType.status,'x_Flat_data_files/1_Social/Outputs/Status_trends_analysis/SulawesiTenggara_20200415/ThreatType.bysett.xlsx')
...
export(PostCodeData.PLOTFORMAT,'x_Flat_data_files/1_Social/Outputs/Status_trends_analysis/SulawesiTenggara_20200415/PostCode_Education_statusplot_data.xlsx')
