...
Flotim.TechReport.MPAvControl <- 
  MPA.TechReport.SigTest.Data[MPA.TechReport.SigTest.Data$MPAID==16,]
...
Flotim.FreqTables <- HHData %>%
  filter(MPAID==16 & Treatment==1) %>%
...
dist.Flotim.DaysUnwell <- 
  ...
  geom_histogram(aes(x=Unwell,y=..density..),
                 bins=20,fill="#5971C7",colour="#262F52",na.rm=T) +
  stat_function(fun=dnorm,args=list(mean=mean(Flotim.TechReport.MPAHouseholdData$Unwell),
                                    sd=sd(Flotim.TechReport.MPAHouseholdData$Unwell)),
                colour="#262F52",size=1,na.rm=T) +
  labs(x="Days Unwell\n(per household)",
       y="Density",
...
qqnorm(Flotim.TechReport.MPAHouseholdData$Unwell)
qqline(Flotim.TechReport.MPAHouseholdData$Unwell,col="green")
                      ...
                      results <- 
                        list(cbind.data.frame(SettlementName=c(as.character(sett.names.Flotim[which(sett.names.Flotim!=median.setts.Flotim[a])]),
                                              ...
                                              rbind.data.frame(t(data.frame(mapply(i=sett.names.Flotim[which(sett.names.Flotim!=median.setts.Flotim[a])],
                                                                                   function(i){
                                                                                     var <- 
                                                                                       unlist(Flotim.TechReport.MPAHouseholdData[Flotim.TechReport.MPAHouseholdData$SettlementName==i |
...
non.parametric.test.MPAvControl.Flotim <-
  data.frame(mapply(a=c("FSIndex","MAIndex","MTIndex","PAIndex","SERate","TimeMarket","DaysUnwell"),
                    function(a){
                      var <- unlist(Flotim.TechReport.MPAvControl[,a])
