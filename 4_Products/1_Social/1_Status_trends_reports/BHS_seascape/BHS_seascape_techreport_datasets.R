...
BHS.level.ContData.status <- 
  cbind.data.frame(MonitoringYear="4 Year Post",
                   MPAID=0,
                   MPAName="Bird's Head Seascape",
                   Techreport.BHSmeans[Techreport.BHSmeans$MonitoringYear=="4 Year Post",-1])

BHS.level.ContData.annex <- 
  cbind.data.frame(MonitoringYear=c("Baseline","2 Year Post","4 Year Post"),
                   MPAID=0,
                   MPAName="Bird's Head Seascape",
                   Techreport.BHSmeans[,-1])

BHS.level.ContData.baseline <-
  cbind.data.frame(MonitoringYear="Baseline",
                   MPAID=0,
                   MPAName="Bird's Head Seascape",
                   Techreport.BHSmeans[Techreport.BHSmeans$MonitoringYear=="Baseline",-c(1,12:13)])
...
BHS.ContData.Techreport.status <- 
  ...
  left_join(BigFive.MPAGroup[BigFive.MPAGroup$MonitoringYear=="4 Year Post" &
                               BigFive.MPAGroup$MPAID<7,c(3,6:15)],
            by="MPAID") %>%
  left_join(Techreport.ByMPA[Techreport.ByMPA$MonitoringYear=="4 Year Post" & 
                               Techreport.ByMPA$MPAID<7,c("MPAID","TimeMarketMean","TimeMarketErr")],
            by="MPAID") %>%
  left_join(Days.unwell.ByMPA[Days.unwell.ByMPA$MonitoringYear=="4 Year Post" &
                                Days.unwell.ByMPA$MPAID<7,c(1,3,4)],
            by="MPAID")
...
BHS.AnnexContData.Techreport <- 
  left_join(BigFive.MPAGroup[BigFive.MPAGroup$MPAID<7,c(3,5)],
            data.frame(MPAID=c(1:6),MPAName=c("Teluk Mayalibit","Teluk Cenderawasih","Kaimana",
                                              "Kofiau dan Pulau Boo","Selat Dampier","Misool Selatan Timur")),
            by="MPAID") %>%
  left_join(BigFive.MPAGroup[BigFive.MPAGroup$MPAID<7,c(3,5,6:15)],
            by=c("MonitoringYear","MPAID")) %>%
  left_join(Techreport.ByMPA[Techreport.ByMPA$MPAID<7,c("MPAID","MonitoringYear","TimeMarketMean","TimeMarketErr")],
            by=c("MonitoringYear","MPAID")) %>%
  left_join(Days.unwell.ByMPA[Days.unwell.ByMPA$MPAID<7,],
            by=c("MonitoringYear","MPAID"))
...
dir.create("x_Flat_data_files/1_Social/Outputs/BHS_level_Analysis")

# ---- 3.1 Define filename for Excel spreadsheet ----

FileName <- paste(paste("x_Flat_data_files/1_Social/Outputs/BHS_level_Analysis/TechReportData--produced",
