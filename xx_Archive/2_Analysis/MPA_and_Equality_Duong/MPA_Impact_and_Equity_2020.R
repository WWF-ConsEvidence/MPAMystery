...
##---1a. Conducting Principle Component Analysis (PCA) for Asset weights: 9 asset items
##---1b. Conducting Principle Component Analysis (PCA) for Asset weights: 6 asset items (combine boats)
##---1c. ConductingPrinciple Component Analysis (PCA) for Poverty-level index (following Andam, Ferraro, Sims 2010, or World Bank document)
...
##---6. Matching settlements using baseine characteristics
##---7. Export data frame to Excel for exploratory Stata analysis

##---8. Descriptive Stats and Evidence for baseline inequality in poverty_index and econ-trend (poverty_index)
##---9. Analysis (tables and plots) for impacts and subgroup differential impacts 
...
pacman::p_load(lfe,cowplot,stargazer,broom,qvalue,psych,factoextra,ineq, stringr, tidytext, tidyverse)
...
# Breaking Age quartile 
age.quart.breaks <- DiD.data %>%
  group_by(IndividualAge_raw) %>%
  summarise(quart1 = quantile(MAIndex_pca, probs = c(.2), na.rm = T),
            quart2 = quantile(MAIndex_pca, probs = c(.4), na.rm = T),
            quart3 = quantile(MAIndex_pca, probs = c(.6), na.rm = T))
...
##------------------------------------------------------------------------##
##------------------------------------------------------------------------##
##--------Processing Last Name to code Indigenuousness--------------------##
...
str_clean <- function(strings) {
  require(dplyr)
  require(tm)
  strings %>% tolower() %>% removePunctuation(preserve_intra_word_dashes = TRUE) %>% stripWhitespace() %>% trim()
}
##----------------##
#library(stringr)
DEMOGRAPHIC_BHS <- DEMOGRAPHIC %>% 
  left_join(select(DiD.data, MPAID, HouseholdID), by="HouseholdID") %>% 
  filter(MPAID%in%c(1:6))

DEMOGRAPHIC_SBS <- DEMOGRAPHIC %>% 
  left_join(select(DiD.data, MPAID, HouseholdID), by="HouseholdID") %>% 
  filter(MPAID%in%c(15:18))

last.names.df<- DEMOGRAPHIC_BHS %>% 
  select(IndividualName) %>% 
  mutate(lastName = word(IndividualName, -1)) %>% 
  mutate(lastName = str_clean(lastName))
# Count frequencies of words for EconomicStatusReason
#library(tidytext)
last.names.rank <- last.names.df %>% 
  count(lastName, sort = TRUE)

# # 2b. Export word frequencies to txt and xls file
# library(foreign)
# library(writexl)
# write_xlsx(last.names.rank, path = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/last_name_rank_BHS", col_names = TRUE, format_headers = TRUE)
...
##---BEGIN---##
##---BEGIN---##
##---1a. Conducting Principle Component Analysis (PCA) for Asset weights: 9 asset items (combine boats)
##---1a. Conducting Principle Component Analysis (PCA) for Asset weights: 9 asset items (combine boats)
...
#----------##
#----------##
##---1c. Poverty Index: Principle Component Analysis (PCA) for Poverty-level index (following Andam, Ferraro, Sims 2010, or World Bank document)
##---1c. Note: Household items + social composition (Male pecentage, ed.level, fuel biomass, FSIndex)

asset.frame.poverty <- DiD.data %>% 
  mutate(CookingFuel.nonBiomass = 1-CookingFuel.Biomass) %>%
  subset(RemoveMA=="No") %>% 
  select(HouseholdID:Treatment, PhoneCombined, TV, Entertain, Satellite, Generator, ed.level, CookingFuel.nonBiomass, FSIndex) %>% 
  na.omit()
summary(asset.frame.poverty)
#Omiting all NAs in assets to compute PCA weights (losing 13 records with NA)

#calculate Z scores for poverty index
asset.frame.poverty <- asset.frame.poverty %>% 
  group_by(MonitoringYear) %>% 
  mutate_at(vars(PhoneCombined: FSIndex), .funs = list(`z`= ~ (.-mean(.,na.rm=T))/sd(.,na.rm = T))) %>% 
  ungroup()
summary(asset.frame.poverty)


#Perform PCA to get eigenvalues (i.e. loading factors) for each asset's weights
poverty.pca <- prcomp(select(asset.frame.poverty, PhoneCombined_z: FSIndex_z), scale = FALSE)

#Get eigenvalues, or variance percentage, for each principle components (i.e. how well each of PCs contribute in explaning the total variance in our asset data set)
poverty.eig.val <- get_eigenvalue(poverty.pca)
poverty.eig.val
fviz_eig(poverty.pca) #a scree plot to illustrate the above
poverty.eig.val.keep <- poverty.eig.val %>% 
  subset(eigenvalue>=1) %>% 
  mutate(pc.contrib = variance.percent/sum(variance.percent)) #keep only PCs with eigenvalue >=1 and find % of explained variance that each PC contributes
poverty.eig.val.keep <- as.data.frame(poverty.eig.val.keep)

#Get the contribution of each asset items in each PCs 
#Importantly, literature use (1) the contribution factors associated with the first component (i.e. Dimension 1) 
#to construct the asset weights in computing composite asset-based index, or (2) all Dimensions with eigenvalue >=1
poverty.weight.frame <- get_pca_var(poverty.pca)$contrib
poverty.weight.frame <- poverty.weight.frame[ ,1:nrow(poverty.eig.val.keep)] 
poverty.weight.frame <- as.data.frame(t(poverty.weight.frame))
poverty.weight.frame <- cbind(poverty.weight.frame,poverty.eig.val.keep) 
...
#Extract weights (scoring factors) using the variance contribution of each assets to the First PC
poverty.weight.pc1 <- poverty.weight.frame[1,] %>% 
  select(PhoneCombined_z: FSIndex_z) 
poverty.weight.pc1

varName<-c("PhoneCombined_z","TV_z","Entertain_z","Satellite_z","Generator_z","ed.level_z","CookingFuel.nonBiomass_z","FSIndex_z")

##note: divide all weights by the highest weight value (standardized against the highest weights)
PhoneCombined_z_w <- poverty.weight.pc1$PhoneCombined_z/poverty.weight.pc1$Satellite_z
TV_z_w <- poverty.weight.pc1$TV_z/poverty.weight.pc1$Satellite_z
Entertain_z_w <- poverty.weight.pc1$Entertain_z/poverty.weight.pc1$Satellite_z
Satellite_z_w <- poverty.weight.pc1$Satellite_z/poverty.weight.pc1$Satellite_z
Generator_z_w <- poverty.weight.pc1$Generator_z/poverty.weight.pc1$Satellite_z
ed.level_z_w <- poverty.weight.pc1$ed.level_z/poverty.weight.pc1$Satellite_z
CookingFuel.nonBiomass_z_w <- poverty.weight.pc1$CookingFuel.nonBiomass_z/poverty.weight.pc1$Satellite_z
FSIndex_z_w <- poverty.weight.pc1$FSIndex_z/poverty.weight.pc1$Satellite_z
#----------##
DiD.data <- DiD.data %>% 
  mutate(CookingFuel.nonBiomass=1-CookingFuel.Biomass) %>% 
  mutate(poverty_index = PhoneCombined*PhoneCombined_z_w + TV*TV_z_w +  Entertain*Entertain_z_w + Satellite*Satellite_z_w + Generator*Generator_z_w + ed.level*ed.level_z_w + CookingFuel.nonBiomass*CookingFuel.nonBiomass_z_w + FSIndex*FSIndex_z_w) 
...
poverty_GINI_Settl <- DiD.data %>% 
  group_by(SettlementID, yearsPost) %>% 
  summarise(MAIndex_GINI_Settl = ineq(poverty_index,type="Gini")) 

DiD.data <- DiD.data %>% 
  left_join(poverty_GINI_Settl, by=c("SettlementID", "yearsPost"))


#Plot the Lorenz curve 
plot(Lc(DiD.data$poverty_index),col="darkred",lwd=2)

hist(DiD.data$MAIndex_GINI_Settl)

##---covariate for settlement's baseline average poverty_index
poverty_index.base.Settl <- DiD.data %>% 
  filter(yearsPost==0) %>% 
  select(SettlementID, poverty_index) %>% 
  group_by(SettlementID) %>% 
  summarise(poverty_index.base = mean(poverty_index, na.rm = TRUE))

DiD.data <- DiD.data %>% 
  left_join(poverty_index.base.Settl, by="SettlementID")
##---END---##
##---END---##
##---END---##
...
DiD.data.matchingCov <- DiD.data %>%
  filter(yearsPost==0) %>% 
  select(MPAID, MonitoringYear, SettlementID, InterviewYear, Treatment, yearsPost, poverty_index, Fisher, TimeMarket, YrResident, EconStatusTrend) %>% 
  group_by(SettlementID) %>%
  summarise(MPAID=first(MPAID),
            Treatment=first(Treatment),
            Fisher = mean(Fisher,na.rm=T), 
            poverty_index = mean(poverty_index,na.rm=T), 
...
DiD.covariate.match <- DiD.data.matchingCov %>% 
  select(poverty_index:EconStatusTrend, Fisher, MPAID) %>% 
  mutate(MPAID=as.integer(MPAID))

# ##-------------------------------------------------------------------------------------------##
# # Exact matching on MPAID (i.e., control and treatment settlements have to come from the same MPA)
# Xexact<-c(0,0,0,0,1)
# 
...
# # summary(m1)
# m2<-Matching::Match(Y=NULL, Tr, X=DiD.covariate.match, M=1, exact=Xexact, replace=TRUE, ties=T)
# #m2$est;m2$est.noadj;m2$se
# summary(m2)
# 
# #### Compute match balance statistics (don't need stats on exact-matched variables) ######
# # Mahanobis
# mb1<-Matching::MatchBalance(Tr~ poverty_index + TimeMarket + EconStatusTrend + Fisher,data=DiD.covariate.match, match.out=m2, ks=TRUE, nboots=1000, digits=3)
# #qqplot(DiD.covariate.match$poverty_index[m2$index.treated],DiD.covariate.match$poverty_index[m2$index.control])
# #abline(0,1,col="red")

##-------------------------------------------------------------------------------------------##
##---pscore matching
# calculate p scores
pscore <- glm(Tr~ poverty_index + TimeMarket + EconStatusTrend + Fisher, data=DiD.covariate.match)$fitted.values
psore_frame = cbind(pscore, DiD.covariate.match$MPAID)
Xexact<-c(0,1)
m_pscore<-Matching::Match(Y=NULL, Tr, X=psore_frame, M=2, exact=Xexact, replace=TRUE, ties=T)
mb2<-Matching::MatchBalance(Tr~ poverty_index + TimeMarket + EconStatusTrend + Fisher,data=DiD.covariate.match, match.out=m_pscore, ks=TRUE, nboots=1000, digits=3)
...
## -------------------------------------------------------------------------------- ##
## -------------------------------------------------------------------------------- ##
##---8. Descriptive Stats and Evidence for baseline inequality in poverty_index and econ-trend (MAIndex_pca)
# Descriptive Statistics
# Descriptive Statistics
# Descriptive Statistics
# Descriptive statistics for cars with automatic transmission
stargazer(subset(DiD.data[c("MAIndex_pca","MAIndex_pca.1","poverty_index","Fisher", "Female", "Ethnic", "Indigenous",
                            "TimeMarket","n.child","ed.level", 
                            "ed.no", "ed.primary", "ed.high", "ed.college",
                            "FSIndex","MTIndex","PAIndex")], DiD.data$yearsPost==0),
          title="Descriptive statistics: Baseline", type = "text", digits=2, out="D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/summary Stat/baseline-all.txt")
...
##---BEGIN---##
##---BEGIN---##
##---8. Descriptive Stats and Evidence for baseline inequality in poverty_index and econ-trend 

DiD.data.sumStat <- DiD.data %>% 
  select(MAIndex_pca, MAIndex_pca.1, poverty_index, HouseholdID:IndividualAge, IndividualAge_raw, EconStatusTrend,MAIndex:SERate, MAIndex_z: Generator_z, Male:wealthQuint, ethDom.highest.MT, HH.Total.count,HH.workingAge.count, HH.U18.count,HHH.age, HHH.ed.level, MAIndex_GINI_Settl, yearsPost.recode)
...
Gender.baseline <- DiD.data.baseline %>% 
  ...
  group_by(Male) %>% 
  summarise(mean= mean(poverty_index, na.rm=TRUE), 
            sd = sd(poverty_index, na.rm=TRUE), 
            median = median(poverty_index, na.rm=TRUE),
            n=length(poverty_index), 
            se = sd/sqrt(n)) %>% 
  mutate(Gender=ifelse(Male==0,"Female","Male"))

test.val <- round(t.test(DiD.data.baseline$poverty_index~DiD.data.baseline$Gender)$statistic, digits = 3)
...
Gender.point.plot <- ggplot(Gender.baseline,aes(x=Gender,y=mean, label=paste0("n= ",n))) + 
  ...
  labs(x="",y="Poverty Index (PCA)", title="Gender (Male vs. Female) ", subtitle=paste0("Baseline means difference t-test = ",test.val, " (median=triangle)"))  
Gender.point.plot
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Gender_baseIneq",".jpg"))
...
test.val <- round(t.test(DiD.data.baseline$poverty_index~DiD.data.baseline$Livelihood)$statistic, digits = 3)
...
Livelihood.point.plot <- ggplot(Livelihood.baseline,aes(x=Livelihood,y=mean, label=paste0("n= ",n))) + 
  ...
  geom_text(aes(y = mean+1.645*se + 0.5),position = position_dodge(0.9),vjust = 0, size=4) +
  geom_line( position = pd) + theme_bw() + theme(legend.position="none") +
  labs(x="",y="Poverty Index (PCA)", title="Livelihood (Fisher vs. Other)", subtitle=paste0("Baseline means difference t-test = ",test.val, " (median=triangle)"))
Livelihood.point.plot
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Livelihood_baseIneq",".jpg"))
...
test.val <- round(t.test(DiD.data.baseline$poverty_index~DiD.data.baseline$Ethnicity)$statistic, digits = 3)
...
Ethnicity.point.plot <- ggplot(Ethnicity.baseline,aes(x=Ethnicity,y=mean, label=paste0("n= ",n))) + 
  ...
  geom_text(aes(y = mean+1.645*se + 0.5),position = position_dodge(0.9),vjust = 0, size=4) +
  geom_line( position = pd) + theme_bw() + theme(legend.position="none") +
  labs(x="",y="Poverty Index (PCA)", title="Ethnicity (Dominant vs. non-Dominant)", subtitle=paste0("Baseline means difference t-test = ",test.val, " (median=triangle)"))
Ethnicity.point.plot
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Ethnicity_baseIneq",".jpg"))
...
test.val <- round(t.test(DiD.data.baseline$poverty_index~DiD.data.baseline$Residence)$statistic, digits = 3)
...
Residence.point.plot <- ggplot(Residence.baseline,aes(x=Residence,y=mean, label=paste0("n= ",n))) + 
  ...
  geom_text(aes(y = mean+1.645*se + 0.5),position = position_dodge(0.9),vjust = 0, size=4) +
  geom_line( position = pd) + theme_bw() + theme(legend.position="none") +
  labs(x="",y="Poverty Index (PCA)", title="Indigenousness (Below vs. Above MPA residence years' median)", subtitle=paste0("Baseline means difference t-test = ",test.val, " (median=triangle)"))
Residence.point.plot
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Residence_baseIneq",".jpg"))
...
test.val <- round(t.test(DiD.data.baseline$poverty_index~DiD.data.baseline$Age_Group)$statistic, digits = 3)
...
Age_Group.point.plot <- ggplot(Age_Group.baseline,aes(x=Age_Group,y=mean, label=paste0("n= ",n))) + 
  ...
  geom_text(aes(y = mean+1.645*se + 0.5),position = position_dodge(0.9),vjust = 0, size=4) +
  geom_line( position = pd) + theme_bw() + theme(legend.position="none") +
  labs(x="",y="Poverty Index (PCA)", title="Age (Old vs. Young)",subtitle=paste0("Baseline means difference t-test = ",test.val, " (median=triangle)")) 
Age_Group.point.plot
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Age_Group_baseIneq",".jpg"))


library(cowplot)
# -- Combine 
plot_grid(Gender.point.plot, Livelihood.point.plot, Ethnicity.point.plot, Residence.point.plot, ncol=2)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_1",".pdf"), width = 12, height = 9)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_1",".png"), width = 12, height = 9)

plot_grid(Gender.point.plot, Livelihood.point.plot, Ethnicity.point.plot, Residence.point.plot, Age_Group.point.plot, ncol=2)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_2",".pdf"), width = 12, height = 12)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_2",".png"), width = 12, height = 12)
...
# --Gender  
Gender.violin <-  ggplot(filter(DiD.data.baseline,!is.na(Gender)),aes(x=Gender,y=poverty_index))+
  geom_violin(trim=T) +
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) +  theme_classic() +
  #stat_summary(fun.y=mean, geom="point", size=4, color="black") +
  labs(x="",y="Poverty Index (PCA)", title="Gender (Male vs. Female)", subtitle=paste0("Baseline distributions"))  +
  ylim(0,20)
Gender.violin
#ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Gender_baseIneq_violin",".jpg"))


# --Livelihood  
Livelihood.violin <-  ggplot(filter(DiD.data.baseline,!is.na(Livelihood)),aes(x=Livelihood,y=poverty_index))+
  geom_violin(trim=T) +
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) +  theme_classic() +
  #stat_summary(fun.y=mean, geom="point", size=4, color="black") +
  labs(x="",y="Poverty Index (PCA)", title="Livelihood (Fishers vs. other)", subtitle=paste0("Baseline distributions"))  +
  ylim(0,20)
Livelihood.violin
#ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Gender_baseIneq_violin",".jpg"))
...
# --Ethnicity  
Ethnicity.violin <-  ggplot(filter(DiD.data.baseline,!is.na(Ethnicity)),aes(x=Ethnicity,y=poverty_index))+
  geom_violin(trim=T) +
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) +  theme_classic() +
  #stat_summary(fun.y=mean, geom="point", size=4, color="black") +
  labs(x="",y="Poverty Index (PCA)", title="Ethnicity (non-Dominant vs. Dominant)", subtitle=paste0("Baseline distributions"))  +
  ylim(0,20)
Ethnicity.violin
#ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Gender_baseIneq_violin",".jpg"))

# --Residence  
Residence.violin <-  ggplot(filter(DiD.data.baseline,!is.na(Residence)),aes(x=Residence,y=poverty_index))+
  geom_violin(trim=T) +
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) +  theme_classic() +
  #stat_summary(fun.y=mean, geom="point", size=4, color="black") +
  labs(x="",y="Poverty Index (PCA)", title="Residence (Below vs. Above MPA medians)", subtitle=paste0("Baseline distributions"))  +
  ylim(0,20)
Residence.violin
#ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Gender_baseIneq_violin",".jpg"))

# --Gender  
Age_Group.violin <-  ggplot(filter(DiD.data.baseline,!is.na(Age_Group)),aes(x=Age_Group,y=poverty_index))+
  geom_violin(trim=T) +
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) +  theme_classic() +
  #stat_summary(fun.y=mean, geom="point", size=4, color="black") +
  labs(x="",y="Poverty Index (PCA)", title="Age_Group (Young vs. Old)", subtitle=paste0("Baseline distributions"))  +
  ylim(0,20)
Age_Group.violin
#ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Gender_baseIneq_violin",".jpg"))
...
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_1_violin",".pdf"), width = 12, height = 9)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_1_violin",".png"), width = 12, height = 9)

plot_grid(Gender.violin, Livelihood.violin, Ethnicity.violin, Residence.violin, Age_Group.violin, ncol=2)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_2_violin",".pdf"), width = 12, height = 12)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Combine_baseIneq_2_violin",".png"), width = 12, height = 12)
...
### -- Triple-Dif aggregate impacts
### -- Triple-Dif aggregate impacts
varNames <- c("poverty_index", "poverty_index.1")

## -- Variable: poverty_index
regValue.gender <- felm(poverty_index  ~ Treatment + Post + Treatment:Post + Female:Treatment + Female:Post + Female:Treatment:Post +
...
regValue.Fisher <- felm(poverty_index  ~ Treatment + Post + Fisher + Treatment:Post + Fisher:Treatment + Fisher:Post + Fisher:Treatment:Post +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data,exactDOF = TRUE)

regValue.ethnic <- felm(poverty_index  ~ Treatment + Post + Ethnic + Treatment:Post + Ethnic:Treatment + Ethnic:Post + Ethnic:Treatment:Post +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data,exactDOF = TRUE)

regValue.indigenous <- felm(poverty_index  ~ Treatment + Post + Indigenous + Treatment:Post + Indigenous:Treatment + Indigenous:Post + Indigenous:Treatment:Post +
                              Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                            | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data,exactDOF = TRUE)


stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups.html", type = "html",
...
stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups.txt", type = "text",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          #order = c(5, 6, 1, 2, 3, 4),
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),          title="Differential Impacts by Social Group (Triple-difference DiD)",
...
## -- Variable: poverty_index.1
regValue.gender <- felm(poverty_index.1  ~ Treatment + Post + Treatment:Post + Female:Treatment + Female:Post + Female:Treatment:Post +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw 
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data,exactDOF = TRUE)

regValue.Fisher <- felm(poverty_index.1  ~ Treatment + Post + Fisher + Treatment:Post + Fisher:Treatment + Fisher:Post + Fisher:Treatment:Post +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data,exactDOF = TRUE)

regValue.ethnic <- felm(poverty_index.1  ~ Treatment + Post + Ethnic + Treatment:Post + Ethnic:Treatment + Ethnic:Post + Ethnic:Treatment:Post +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data,exactDOF = TRUE)

regValue.indigenous <- felm(poverty_index.1  ~ Treatment + Post + Indigenous + Treatment:Post + Indigenous:Treatment + Indigenous:Post + Indigenous:Treatment:Post +
                              Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                            | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data,exactDOF = TRUE)


stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups-noBoat.html", type = "html",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          #order = c(5, 6, 1, 2, 3, 4),
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),          title="Differential Impacts by Social Group (Triple-difference DiD)",
          align=TRUE, dep.var.labels="Poverty Index (PCA) [not including Boat]",
          add.lines = list(c("Settlement FE",rep("Yes",4)), c("MPA x Year FE",rep("Yes",4))))



stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups-noBoat.txt", type = "text",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          #order = c(5, 6, 1, 2, 3, 4),
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),          title="Differential Impacts by Social Group (Triple-difference DiD)",
...
stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups-BHS.html", type = "html",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          #order = c(5, 6, 1, 2, 3, 4),
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),          title="Differential Impacts by Social Group (Triple-difference DiD)",
...
stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups-BHS.txt", type = "text",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          #order = c(5, 6, 1, 2, 3, 4),
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),          title="Differential Impacts by Social Group (Triple-difference DiD)",
...
regValue.gender <- felm(poverty_index  ~ Treatment + yearsPostF + Treatment:yearsPostF + Female:Treatment + Female:yearsPostF + Female:Treatment:yearsPostF +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.BHS,exactDOF = TRUE)

regValue.Fisher <- felm(poverty_index  ~ Treatment + yearsPostF + Treatment:yearsPostF + Fisher:Treatment + Fisher:yearsPostF + Fisher:Treatment:yearsPostF +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.BHS,exactDOF = TRUE)

regValue.ethnic <- felm(poverty_index  ~ Treatment + yearsPostF  + Treatment:yearsPostF + Ethnic:Treatment + Ethnic:yearsPostF + Ethnic:Treatment:yearsPostF +
                          Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                        | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.BHS,exactDOF = TRUE)

regValue.indigenous <- felm(poverty_index  ~ Treatment + yearsPostF + Treatment:yearsPostF + Indigenous:Treatment + Indigenous:yearsPostF + Indigenous:Treatment:yearsPostF +
                              Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw 
                            | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.BHS,exactDOF = TRUE)


stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups-BHS-t2t4.html", type = "html",
...
stargazer(regValue.gender, regValue.Fisher, regValue.ethnic, regValue.indigenous,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Triple-Dif-subGroups-BHS-t2t4.txt", type = "text",
...
MAIndex_pca.Gender.plot <- ggplot(filter(model.out.gender1,Response=="poverty_index"),aes(x=Group,y=estimate)) + 
...
ggsave("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Impact_plots/Gender-poverty_index.png",width = 12, height = 6)


poverty_index.Livelihood.plot <- ggplot(filter(model.out.Fisher1,Response=="poverty_index"),aes(x=Group,y=estimate)) + 
  geom_bar(stat="identity", position =pd, fill="darkgray", width = 0.6)+ 
  theme_bw() + theme(legend.position="none") +
  geom_line( position = pd) + 
...
ggsave("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Impact_plots/Fisher-poverty_index.png",width = 12, height = 6)


poverty_index.Ethnic.plot <- ggplot(filter(model.out.ethnic1,Response=="poverty_index"),aes(x=Group,y=estimate)) + 
  geom_bar(stat="identity", position =pd, fill="darkgray", width = 0.6)+ 
  theme_bw() + theme(legend.position="none") +
  geom_line( position = pd) + 
  geom_errorbar(aes(ymin=estimate-1.645*std.error, ymax=estimate+1.645*std.error), width=0.4, position = pd ) +
...
ggsave("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Impact_plots/Ethnic-poverty_index.png",width = 12, height = 6)


poverty_index.Indigenous.plot <- ggplot(filter(model.out.indigenous1,Response=="poverty_index"),aes(x=Group,y=estimate)) + 
  geom_bar(stat="identity", position =pd, fill="darkgray", width = 0.6)+ 
  theme_bw() + theme(legend.position="none") +
  geom_line( position = pd) + 
  geom_errorbar(aes(ymin=estimate-1.645*std.error, ymax=estimate+1.645*std.error), width=0.4, position = pd ) +
...
ggsave("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Impact_plots/Indigenous-poverty_index.png",width = 12, height = 6)

#--Combine
plot_grid(poverty_index.Gender.plot, poverty_index.Livelihood.plot, poverty_index.Ethnic.plot, poverty_index.Indigenous.plot, ncol=2)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Impact_plots/Combine-poverty_index",".pdf"), width = 12, height = 9)
ggsave(paste0("D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/plots/Impact_plots/Combine-poverty_index",".png"), width = 12, height = 9)
...
# 
# 
# ##----------Individual Seascape Impact plots-------------------##
# ##----------Individual Seascape Impact plots-------------------##
# ##----------Individual Seascape Impact plots-------------------##
# 
...
#   ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_FS_",mpa,".jpg"),width = 12, height = 6)
#   
...
#   ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_MT_",mpa,".jpg"),width = 12, height = 6)
#   
#   
...
#   ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_MA_",mpa,".jpg"),width = 12, height = 6)
#   
#   
#   
...
#   ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_PA_",mpa,".jpg"),width = 12, height = 6)
#   
#   
#   
...
#   ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_SE_",mpa,".jpg"),width = 12, height = 6)
#   
#   
#   #library(cowplot)
#   #Combine "regular BigFive"
#   plot_grid(FS.plot,MT.plot,PA.plot,SE.plot,MA.plot,ncol=3)
#   ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_BigFive_",mpa,".jpg"),width = 12, height = 6)
#   
...
## -- all BHS and SBS -- mechanism: intensive participation of social groups
regValue.gender1 <- felm(poverty_index  ~ Treatment + Post + Treatment:Post + Female:Treatment + Female:Post + Female:Treatment:Post +
                           Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                         | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.Female.m.PartiAbove,exactDOF = TRUE)

regValue.Fisher1 <- felm(poverty_index  ~ Treatment + Post + Fisher + Treatment:Post + Fisher:Treatment + Fisher:Post + Fisher:Treatment:Post +
                           Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw  
                         | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.Fisher.m.PartiAbove,exactDOF = TRUE)

regValue.ethnic1 <- felm(poverty_index  ~ Treatment + Post + Ethnic + Treatment:Post + Ethnic:Treatment + Ethnic:Post + Ethnic:Treatment:Post +
                           Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw  
                         | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.Ethnic.m.PartiAbove,exactDOF = TRUE)

regValue.indigenous1 <- felm(poverty_index  ~ Treatment + Post + Indigenous + Treatment:Post + Indigenous:Treatment + Indigenous:Post + Indigenous:Treatment:Post +
                               Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw  
                             | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.Indigenous.m.PartiAbove,exactDOF = TRUE)

##-----------------------------------------##
regValue.gender2 <- felm(poverty_index  ~ Treatment + Post + Treatment:Post + Female:Treatment + Female:Post + Female:Treatment:Post +
                           Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw   
                         | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.Female.m.PartiBelow,exactDOF = TRUE)

regValue.Fisher2 <- felm(poverty_index  ~ Treatment + Post + Fisher + Treatment:Post + Fisher:Treatment + Fisher:Post + Fisher:Treatment:Post +
                           Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw  
                         | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.Fisher.m.PartiBelow,exactDOF = TRUE)

regValue.ethnic2 <- felm(poverty_index  ~ Treatment + Post + Ethnic + Treatment:Post + Ethnic:Treatment + Ethnic:Post + Ethnic:Treatment:Post +
                           Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw  
                         | SettlementID + MPAID:InterviewYear | 0 | SettlementID, data=DiD.data.Ethnic.m.PartiBelow,exactDOF = TRUE)

regValue.indigenous2 <- felm(poverty_index  ~ Treatment + Post + Indigenous + Treatment:Post + Indigenous:Treatment + Indigenous:Post + Indigenous:Treatment:Post +
                               Female + Fisher + Ethnic + Indigenous + n.child + ed.level + YrResident + IndividualAge_raw  
...
stargazer(regValue.gender1, regValue.Fisher1, regValue.ethnic1, regValue.indigenous1,regValue.gender2, regValue.Fisher2, regValue.ethnic2, regValue.indigenous2,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Moderators/Triple-Dif-subGroups-PartiSpecific-m.html", type = "html",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          ...
          title="Mechanism: Baseline Participation Intensity (by each group)", align=TRUE, dep.var.labels="Poverty Index (PCA)",
          add.lines = list(c("Settlement FE",rep("Yes",8)), c("MPA x Year FE",rep("Yes",8)), c("Sample",rep("Above Median",4),rep("Below Median",4))))

stargazer(regValue.gender1, regValue.Fisher1, regValue.ethnic1, regValue.indigenous1,regValue.gender2, regValue.Fisher2, regValue.ethnic2, regValue.indigenous2,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Moderators/Triple-Dif-subGroups-PartiSpecific-m.txt", type = "text",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),         
          title="Mechanism: Baseline Participation Intensity (by each group)", align=TRUE, dep.var.labels="Poverty Index (PCA)",
...
stargazer(regValue.gender1, regValue.Fisher1, regValue.ethnic1, regValue.indigenous1,regValue.gender2, regValue.Fisher2, regValue.ethnic2, regValue.indigenous2,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Moderators/Triple-Dif-subGroups-PartiSpecific-pct.html", type = "html",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),         
          title="Mechanism: Baseline Participation Likelihood (by each group)", align=TRUE, dep.var.labels="Poverty Index (PCA)",
          add.lines = list(c("Settlement FE",rep("Yes",8)), c("MPA x Year FE",rep("Yes",8)), c("Sample",rep("Above Median",4),rep("Below Median",4))))

stargazer(regValue.gender1, regValue.Fisher1, regValue.ethnic1, regValue.indigenous1,regValue.gender2, regValue.Fisher2, regValue.ethnic2, regValue.indigenous2,
          out = "D:/Dropbox/MPA_research/Paper 1-MPA and Equity/results/2020/tables/Moderators/Triple-Dif-subGroups-PartiSpecific-pct.txt", type = "text",
          keep = c("Treatment:Post",	"Treatment:Post:Female","Treatment:Post:Fisher","Treatment:Post:Ethnic","Treatment:Post:Indigenous", 
                   "\\bFemale\\b", "\\bFisher1\\b", "\\bEthnic\\b", "\\bIndigenous\\b",  "n.child", "ed.level", "YrResident", "IndividualAge_raw"), 
          covariate.labels=c("Female", "Fisher", "Ethnic", "Indigenous",  "Number of Children", "Education Level", "Years of Residence", "Household Head's Age"),         
          title="Mechanism: Baseline Participation Likelihood (by each group)", align=TRUE, dep.var.labels="Poverty Index (PCA)",
