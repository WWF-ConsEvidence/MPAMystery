# 
# code:  Aggregated BHS-level Technical Report Significance Tests FOR CONTROL HOUSEHOLDS, AT BHS LEVEL
# 
...
BHS.Seascape.TechReport.SigTest.Data <- 
  HHData[HHData$MPAID<7,] %>%
  mutate(MPAName=ifelse(MPAID==1, "Teluk Mayalibit",
                        ifelse(MPAID==2, "Teluk Cenderawasih",
                               ifelse(MPAID==3, "Kaimana",
                                      ifelse(MPAID==4, "Kofiau dan Pulau Boo",
                                             ifelse(MPAID==5, "Selat Dampier",
                                                    ifelse(MPAID==6, "Misool Selatan Timur",
                                                           NA)))))))

# - "BHS Household Data" dataset
BHS.TechReport.SeascapeHouseholdData <- 
  left_join(BHS.Seascape.TechReport.SigTest.Data[BHS.Seascape.TechReport.SigTest.Data$Treatment==0 &
                                                   BHS.Seascape.TechReport.SigTest.Data$MonitoringYear== "4 Year Post",], 
            Days.unwell[Days.unwell$MPAID<7 & 
...
BHS.TechReport.MPAMeans <- 
  left_join(BigFive.ControlGroup[BigFive.ControlGroup$MonitoringYear=="4 Year Post" &
                                   BigFive.ControlGroup$MPAID<7,-c(1:2)],
            Techreport.Control[Techreport.Control$MonitoringYear=="4 Year Post" &
                                 Techreport.Control$MPAID<7,
                             c("MPAID","TimeMarketMean")],  
            by=c("MPAID")) %>%
  left_join(Days.unwell.Control[Days.unwell.Control$MonitoringYear=="4 Year Post" &
                                  Days.unwell.Control$MPAID<7,c("MPAID","UnwellMean")],
            by="MPAID") %>%
...
colnames(BHS.TechReport.MPAMeans) <- c(colnames(BHS.TechReport.MPAMeans)[1:3],
                                       ...
                                       "MTIndex","MTErr","SERate","SEErr","TimeMarket","DaysUnwell","MPAName")

# - "Trend" dataset
BHS.Trend.Data <- 
  left_join(BHS.Seascape.TechReport.SigTest.Data[BHS.Seascape.TechReport.SigTest.Data$Treatment==0,],
            Days.unwell[Days.unwell$Treatment==0 &
                          Days.unwell$MPAID<7,1:2],
            by="HouseholdID") 
...
BHS.Baseline.TechReport.SeascapeHouseholdData <- 
  left_join(BHS.Seascape.TechReport.SigTest.Data[BHS.Seascape.TechReport.SigTest.Data$Treatment==0 &
            ...
            Days.unwell[Days.unwell$MonitoringYear=="Baseline" &
                          Days.unwell$Treatment==0,
                                  c("HouseholdID","DaysUnwell")],
            by="HouseholdID")
...
BHS.Baseline.TechReport.MPAMeans <- 
  left_join(BigFive.ControlGroup[BigFive.ControlGroup$MonitoringYear=="Baseline" &
                                   BigFive.ControlGroup$MPAID<7,-c(1:2)],
            Techreport.Control[Techreport.Control$MonitoringYear=="Baseline" &
                                 Techreport.Control$MPAID<7,
                             c("MPAID","TimeMarketMean")],  
            by=c("MPAID")) %>%
  left_join(Days.unwell.Control[Days.unwell.Control$MonitoringYear=="Baseline" &
                                  Days.unwell.Control$MPAID<7,c("MPAID","UnwellMean")],
            by="MPAID") %>%
  left_join(data.frame(MPAID=c(1:6),MPAName=c("Teluk Mayalibit","Teluk Cenderawasih","Kaimana",
                                              "Kofiau dan Pulau Boo","Selat Dampier","Misool Selatan Timur")),
...
even.number.MPA.function.BHS <- 
  mapply(a=BHS.TechReport.MPAMeans[,c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarket","DaysUnwell")],
         b=BHS.TechReport.SeascapeHouseholdData[,c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarket","DaysUnwell")],
         function(a,b){
...
median.MPA.BHS <- 
  mapply(i=BHS.TechReport.MPAMeans[,c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarket","DaysUnwell")],
...
non.parametric.test.MPA.BHS <- 
  data.frame(mapply(a=c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarket","DaysUnwell"),
                    function(a){
                      results <- 
...
sigvals.BHS <- 
  cbind.data.frame(non.parametric.test.MPA.BHS[order(non.parametric.test.MPA.BHS$"FSIndex.MPAName"),
                   ...
                   non.parametric.test.MPA.BHS[order(non.parametric.test.MPA.BHS$"TimeMarket.MPAName"),
                                                        c("TimeMarket.MPAName","TimeMarket.p.value")],
...
trend.non.parametric.test.BHS <- 
  data.frame(mapply(i=BHS.Trend.Data[,c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarket","DaysUnwell")],
                    function(i){
