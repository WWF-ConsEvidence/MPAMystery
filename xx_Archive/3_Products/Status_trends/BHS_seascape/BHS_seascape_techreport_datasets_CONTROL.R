...
BHS.level.ContData.status <- 
  cbind.data.frame(MonitoringYear="4 Year Post",
                   MPAID=0,
                   MPAName="Bird's Head Seascape",
                   Techreport.BHSmeans.Control[Techreport.BHSmeans.Control$MonitoringYear=="4 Year Post",-1])

BHS.level.ContData.annex <- 
  cbind.data.frame(MonitoringYear=c("Baseline","2 Year Post","4 Year Post"),
                   MPAID=0,
                   MPAName="Bird's Head Seascape",
                   Techreport.BHSmeans.Control[,-1])

BHS.level.ContData.baseline <-
  cbind.data.frame(MonitoringYear="Baseline",
                   MPAID=0,
                   MPAName="Bird's Head Seascape",
                   Techreport.BHSmeans.Control[Techreport.BHSmeans.Control$MonitoringYear=="Baseline",-c(1,12:13)])
...
BHS.ContData.Techreport.status <- 
  ...
  left_join(BigFive.ControlGroup[BigFive.ControlGroup$MonitoringYear=="4 Year Post" &
                                   BigFive.ControlGroup$MPAID<7,c(3,6:15)],
            by="MPAID") %>%
  left_join(Techreport.Control[Techreport.Control$MonitoringYear=="4 Year Post" & 
                                 Techreport.Control$MPAID<7,c("MPAID","TimeMarketMean","TimeMarketErr")],
            by="MPAID") %>%
  left_join(Days.unwell.Control[Days.unwell.Control$MonitoringYear=="4 Year Post" &
                                  Days.unwell.Control$MPAID<7,c(1,3,4)],
            by="MPAID")
...
BHS.ContData.Techreport.baseline <- 
  data.frame(MPAID=c(1:6),MPAName=c("Teluk Mayalibit","Teluk Cenderawasih","Kaimana",
                                    "Kofiau dan Pulau Boo","Selat Dampier","Misool Selatan Timur")) %>%
  left_join(BigFive.ControlGroup[BigFive.ControlGroup$MonitoringYear=="Baseline",c(3,6:15)],
            by="MPAID") %>%
  left_join(Days.unwell.Control[Days.unwell.Control$MonitoringYear=="Baseline",c(1,3,4)],
            by="MPAID")
...
BHS.AnnexContData.Techreport <- 
  left_join(BigFive.ControlGroup[BigFive.ControlGroup$MPAID<7,c(3,5)],
            data.frame(MPAID=c(1:6),MPAName=c("Teluk Mayalibit","Teluk Cenderawasih","Kaimana",
                                              "Kofiau dan Pulau Boo","Selat Dampier","Misool Selatan Timur")),
            by="MPAID") %>%
  left_join(BigFive.ControlGroup[BigFive.ControlGroup$MPAID<7,c(3,5,6:15)],
            by=c("MonitoringYear","MPAID")) %>%
  left_join(Techreport.Control[Techreport.Control$MPAID<7,c("MPAID","MonitoringYear","TimeMarketMean","TimeMarketErr")],
            by=c("MonitoringYear","MPAID")) %>%
  left_join(Days.unwell.Control[Days.unwell.Control$MPAID<7,],
            by=c("MonitoringYear","MPAID"))
...
FileName <- paste(paste("x_Flat_data_files/1_Social/Outputs/BHS_level_Analysis/CONTROL_TechReportData--produced",
