...
att.significance <- function(outcomes, HHData, weights = NULL, grouping_var = NULL, conf = NULL, sig.test = NULL, p.value = NULL) {
...
# Developing initial data frame 
  ...
  if(!is.na(group)) {x <- dplyr::group_by(x, get(group))} else {x <- x}
