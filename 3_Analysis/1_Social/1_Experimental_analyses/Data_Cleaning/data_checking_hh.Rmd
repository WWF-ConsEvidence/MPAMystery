...
print(paste0("Values out of range for variables with Yes/No choices for Household ID", yes_no_var_index$householdid))
print(yes_no_var_index)
```
```{r}
rights_manage_index <- 
    filter(hh_unipa, !rightsmanage%in%c(yes_no_codes, skipcodes)) %>%
...
* TREATMENT_CHOICES 
```{r}
treatment_codes <- c(0:1) # +skip codes
```

* ORGANIZATION_POSITION_CHOICES
```{r}
org_pos_codes <- c(1:2) # +skip codes
```
