...
soc.coord <- site.sf %>%
  ...
  bind_rows(pupier.sf)
head(soc.coord) 
filter(soc.coord,SettlementID==67)
...
##---Spec 1-3: 1to3 matching DiD; varying DiD specs
##---Spec 4-6: Replicate of main spec (Spec 1); varying matching (1to1, 1to2, and coarse)
regValue.list <-list()
...
for (i in varNames) {
  print(i)
    ...
    ##----------------------------------------------------------------------------##
    ## Specification 2 (base and relaxing pair.id FEs)
    regValue <- felm(Y  ~  Treatment + Post + Treatment:Post + 
                       n.child  + ed.level +  dom.eth + YearsResident + IndividualGender + IndividualAge + PrimaryLivelihood.bin
    ...
    ##----------------------------------------------------------------------------##
    ## Specification 3 (base and adding MPAIDxInterviewYear FEs)
    regValue <- felm(Y  ~  Treatment + Post + Treatment:Post + 
                           PrimaryLivelihood.bin
                     | SettlementID  + pair.id + InterviewYear | 0 | SettlementID + pair.id, data=DiD.data.SettlMatch,exactDOF = TRUE, weights = w)
    regValue.3 <- regValue
    ...
    ##----------------------------------------------------------------------------##
    ## Specification 6  (coarse matching; note: no more pair.id FE and cluster; also no "weight")
...
names(master.reg.list$FSIndex_z) <- paste0("Specification ",seq(1:6))
names(master.reg.list$MAIndex_z) <- paste0("Specification ",seq(1:6))
names(master.reg.list$MTIndex_z) <- paste0("Specification ",seq(1:6))
names(master.reg.list$PAIndex_z) <- paste0("Specification ",seq(1:6))
names(master.reg.list$SERate_z) <- paste0("Specification ",seq(1:6))
...
##save texts -- with FEs
stargazer(master.reg.list$FSIndex_z[c("Specification 1","Specification 2","Specification 3","Specification 4","Specification 5","Specification 6")],
          ...
          add.lines = list(c("Settlement Matching",rep("1-to-3",3), "1-to-2", "1-to-1", "none"),
                           c("Household Covariates","Yes","Yes","No",rep("Yes",3)),
                           c("Pair Cluster (s.e.)","Yes", "No", rep("Yes",4)),
                           c("Settlement Cluster (s.e.)",rep("Yes",6)),
                           c("Settlement, Year, Pair FEs",rep("Yes",6))))

stargazer(master.reg.list$PAIndex_z[c("Specification 1","Specification 2","Specification 3","Specification 4","Specification 5","Specification 6")],
          out = "D:/Dropbox/MPA_research/Paper 0-MPA Impact BHS/Settlement_matching/tables/DiD-seascape-z-Sensitivity_PA.txt", type = "text", 
          df = FALSE, notes.append = FALSE, star.cutoffs = NA, omit.table.layout = "n", 
          keep = c("Treatment:Post1"), covariate.labels=c("Treatment X Post"),
          title="DiD Regression Results",
          align=TRUE, column.labels=names(master.reg.list$PAIndex_z), dep.var.labels="Culture (Place Attachment Index)",
          add.lines = list(c("Settlement Matching",rep("1-to-3",3), "1-to-2", "1-to-1", "none"),
                           c("Household Covariates","Yes","Yes","No",rep("Yes",3)),
                           c("Pair Cluster (s.e.)","Yes", "No", rep("Yes",4)),
                           c("Settlement Cluster (s.e.)",rep("Yes",6)),
...
stargazer(master.reg.list$MTIndex_z[c("Specification 1","Specification 2","Specification 3","Specification 4","Specification 5","Specification 6")],
          out = "D:/Dropbox/MPA_research/Paper 0-MPA Impact BHS/Settlement_matching/tables/DiD-seascape-z-Sensitivity_MT.txt", type = "text", 
          df = FALSE, notes.append = FALSE, star.cutoffs = NA, omit.table.layout = "n", 
          keep = c("Treatment:Post1"), covariate.labels=c("Treatment X Post"),
          title="DiD Regression Results",
          align=TRUE, column.labels=names(master.reg.list$MTIndex_z), dep.var.labels="Political Empowerment (Marine Tenure Index)",
          add.lines = list(c("Settlement Matching",rep("1-to-3",3), "1-to-2", "1-to-1", "none"),
                           c("Household Covariates","Yes","Yes","No",rep("Yes",3)),
                           c("Pair Cluster (s.e.)","Yes", "No", rep("Yes",4)),
                           c("Settlement Cluster (s.e.)",rep("Yes",6)),
...
stargazer(master.reg.list$MAIndex_z[c("Specification 1","Specification 2","Specification 3","Specification 4","Specification 5","Specification 6")],
          out = "D:/Dropbox/MPA_research/Paper 0-MPA Impact BHS/Settlement_matching/tables/DiD-seascape-z-Sensitivity_MA.txt", type = "text", 
          df = FALSE, notes.append = FALSE, star.cutoffs = NA, omit.table.layout = "n", 
          keep = c("Treatment:Post1"), covariate.labels=c("Treatment X Post"),
          title="DiD Regression Results",
          align=TRUE, column.labels=names(master.reg.list$MAIndex_z), dep.var.labels="Economic Wealth (Material Assets Index)",
          add.lines = list(c("Settlement Matching",rep("1-to-3",3), "1-to-2", "1-to-1", "none"),
                           c("Household Covariates","Yes","Yes","No",rep("Yes",3)),
                           c("Pair Cluster (s.e.)","Yes", "No", rep("Yes",4)),
                           c("Settlement Cluster (s.e.)",rep("Yes",6)),
...
stargazer(master.reg.list$SERate_z[c("Specification 1","Specification 2","Specification 3","Specification 4","Specification 5","Specification 6")],
          out = "D:/Dropbox/MPA_research/Paper 0-MPA Impact BHS/Settlement_matching/tables/DiD-seascape-z-Sensitivity_SE.txt", type = "text", 
          df = FALSE, notes.append = FALSE, star.cutoffs = NA, omit.table.layout = "n", 
          keep = c("Treatment:Post1"), covariate.labels=c("Treatment X Post"),
          title="DiD Regression Results",
          align=TRUE, column.labels=names(master.reg.list$SERate_z), dep.var.labels="Education (School Enrolment Rate)",
          add.lines = list(c("Settlement Matching",rep("1-to-3",3), "1-to-2", "1-to-1", "none"),
                           c("Household Covariates","Yes","Yes","No",rep("Yes",3)),
                           c("Pair Cluster (s.e.)","Yes", "No", rep("Yes",4)),
                           c("Settlement Cluster (s.e.)",rep("Yes",6)),
