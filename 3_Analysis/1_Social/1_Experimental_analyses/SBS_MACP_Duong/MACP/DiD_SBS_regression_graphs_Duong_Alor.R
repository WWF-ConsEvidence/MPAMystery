...
DiD.data.Alor <- DiD.data %>% 
  filter(MPAID ==15)

for (i in 1:ncol(DiD.data.Alor)){
  
  print(paste0(names(DiD.data.Alor)[i]," (",class(DiD.data.Alor[,i]),")"))
  
  if(length(unique(DiD.data.Alor[,i]))>25 & class(DiD.data.Alor[,i])!="character"){
    print(summary(DiD.data.Alor[,i]))
  }
  
  else{
    if (length(unique(DiD.data.Alor[,i]))<25){
      print(table(DiD.data.Alor[,i]))}
    else{
      print(head(DiD.data.Alor[,i]))}
  }
}
...
export(model.out.MACP.impact1 %>% 
       ...
       "D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/macp_plots_output.csv")
...
export(model.out.MACP.impact1 %>% 
         ...
         arrange(MPAID,Response), 
       "D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/macp_plots_output_standardized_Big5.csv")
...
export(model.out.MACP.impact1.subAsset %>% 
         select(term, estimate, Response, MPAID, std.error, Group, MPAName_short) %>%  
         arrange(MPAID,Response), 
       "D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/macp_plots_output_Asset_subClasses.csv")
...
for (mpa in MPA_name) {
  ...
  FS.plot <- ggplot(filter(mpa.impact.data,Response=="FSIndex_z"),aes(x=term,y=estimate)) + 
    ...
    geom_hline(yintercept = 0, linetype = "dashed") +
    labs(x="",y="Impact estimate", title="Food Security") 
  ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_FS_z_",mpa,".jpg"),width = 12, height = 6)
  ...
  MT.plot <- ggplot(filter(mpa.impact.data,Response=="MTIndex_z"),aes(x=term,y=estimate)) + 
    ...
    geom_hline(yintercept = 0, linetype = "dashed") +
    labs(x="",y="Impact estimate", title="Marine Tenure")  
  ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_MT_z_",mpa,".jpg"),width = 12, height = 6)
  ...
  MA.plot <- ggplot(filter(mpa.impact.data,Response=="MAIndex_z"),aes(x=term,y=estimate)) + 
    ...
    geom_hline(yintercept = 0, linetype = "dashed") +
    labs(x="",y="Impact estimate", title="Material Assets")  
  ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_MA_z_",mpa,".jpg"),width = 12, height = 6)
  ...
  PA.plot <- ggplot(filter(mpa.impact.data,Response=="PAIndex_z"),aes(x=term,y=estimate)) + 
    ...
    geom_hline(yintercept = 0, linetype = "dashed") +
    labs(x="",y="Impact estimate", title="Place Attachment")  
  ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_PA_z_",mpa,".jpg"),width = 12, height = 6)
  ...
  SE.plot <- ggplot(filter(mpa.impact.data,Response=="SERate_z"),aes(x=term,y=estimate)) + 
    ...
    geom_hline(yintercept = 0, linetype = "dashed") +
    labs(x="",y="Impact estimate", title="School Enrollment")  
  ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_SE_z_",mpa,".jpg"),width = 12, height = 6)
  
  #Combine "standardize BigFive"
  plot_grid(FS.plot,MT.plot,PA.plot,SE.plot,MA.plot,ncol=3)
  ggsave(paste0("D:/Dropbox/1_Social/Outputs/impact_analysis/Alor/plots/",mpa,"/DiD_BigFive_z_",mpa,".jpg"),width = 12, height = 6)
