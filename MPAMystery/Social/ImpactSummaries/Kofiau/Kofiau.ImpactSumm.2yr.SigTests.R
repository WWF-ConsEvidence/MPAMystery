...
Kof.BigFive.MPAimpact.summ.st <- cbind.data.frame(rep.int(c("MPA","Control"),2),
                                                  c(rep("Baseline",2),rep("2 Year Post",2)),
                                                  rbind.data.frame(c(summarise(Kof.2yr.HHimpacts,
...
colnames(Kof.BigFive.MPAimpact.summ.st) <- c("Treatment","Year",colnames(Kof.2yr.impacts[,2:11]))
Kof.BigFive.MPAimpact.summ.st$Treatment <- factor(Kof.BigFive.MPAimpact.summ.st$Treatment,
                                                  levels=c("MPA","Control"),
                                                  ordered=T)
Kof.BigFive.MPAimpact.summ.st$Year <- factor(Kof.BigFive.MPAimpact.summ.st$Year,
                                             levels=c("Baseline","2 Year Post","4 Year Post"),
                                             ordered=T)

# --- Impact plot dataset
Kof.2yr.HHimpacts <- impact.2yr[impact.2yr$MPAID==4 & !is.na(impact.2yr$MPAID),]
Kof.2yr.impacts <- cbind.data.frame(c("MPA","Control"),
                                    rbind.data.frame(c(summarise(Kof.2yr.HHimpacts,
...
colnames(Kof.2yr.impacts) <- c("Treatment",colnames(Kof.2yr.impacts[2:11]))
Kof.2yr.impacts$Treatment <- factor(Kof.2yr.impacts$Treatment,
                                    levels=Kof.2yr.impacts$Treatment,
                                    ordered=T)

# --- Snapshot impact plots (all five indicators on one plot, standardized impacts)
Kof.2yr.std.impacts <- subset(std.impact.2yr[std.impact.2yr$MPA=="Kofiau dan Pulau Boo MPA",])
Kof.2yr.std.impacts$MPA <- factor(Kof.2yr.std.impacts$MPA,
                                  labels="Kofiau")

# 2.2 Dataset for statistical analysis
Kof.2yr.HHimpacts.1 <- Kof.2yr.HHimpacts[,c(2,5,9,12,16,19,23,26,30,33)]
colnames(Kof.2yr.HHimpacts.1) <- c("FS","FS","MA","MA","PA","PA","MT","MT","SE","SE")
Kof.BigFive <- cbind.data.frame(c(rep("Baseline",length(Kof.2yr.HHimpacts.1[,1])),
                                  rep("2 Year Post",length(Kof.2yr.HHimpacts.1[,1]))),
                                rbind.data.frame(c(Kof.2yr.HHimpacts.1[,c(1,3,5,7,9)]),
                                                 c(Kof.2yr.HHimpacts.1[,c(2,4,6,8,10)])))
colnames(Kof.BigFive) <- c("Year","FS","MA","PA","MT","SE")
...
Kof.sig.labs <- data.frame(xaxis.st.baselabs=rep(paste("Baseline\nscore\n","(",
                           ...
                           snapshot.labs=mapply(i=p.vals.kofiau[order(p.vals.kofiau$Indicator),"p.val"],
                           ...
                           impact.labs=sapply(p.vals.kofiau[order(p.vals.kofiau$Indicator),"p.val"],
                                              function(i){
                                                c(ifelse(i<0.01,"*\n*\n*",
...
Kof.sig.pos <- data.frame(mapply(i=Kof.2yr.impacts[Kof.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
                                 ...
                                 outcome.sd=c(sd(Kof.2yr.HHimpacts[,c(21)],na.rm=T),
                                              (sd(Kof.2yr.HHimpacts[,c(35)],na.rm=T)/100),
                                              sapply(Kof.2yr.HHimpacts[,c(28,6,13)],sd,na.rm=T)),
