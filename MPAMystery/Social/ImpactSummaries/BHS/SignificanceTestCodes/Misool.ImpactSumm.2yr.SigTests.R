...
Mis.BigFive.summary <- summarise.bigfive(impact.2yr[impact.2yr$MPAID==6 & !is.na(impact.2yr$MPAID),])

# - Status & trend plot dataset, using matched data
Mis.2yr.status.trend <- Mis.BigFive.summary[1:4,]


# - Impact plot dataset
Mis.2yr.impacts <- Mis.BigFive.summary[5:6,c(1,3:12)]
...
Mis.BigFive <- rbind.data.frame(Mis.BigFive.t0,Mis.BigFive.t2)
...
Mis.sig.pos <- data.frame(mapply(i=Mis.2yr.impacts[Mis.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
                                 ...
                                 outcome.sd=c(sd(impact.2yr[impact.2yr$MPAID==6 & !is.na(impact.2yr$MPAID),22],na.rm=T),
                                              (sd(impact.2yr[impact.2yr$MPAID==6 & !is.na(impact.2yr$MPAID),36],na.rm=T)/100),
                                              sapply(impact.2yr[impact.2yr$MPAID==6 & !is.na(impact.2yr$MPAID),c(29,7,14)],sd,na.rm=T)),
