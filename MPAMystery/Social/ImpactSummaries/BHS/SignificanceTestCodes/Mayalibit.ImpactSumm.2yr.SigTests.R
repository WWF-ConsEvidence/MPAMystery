...
Maya.BigFive.summary <- summarise.bigfive(impact.2yr[impact.2yr$MPAID==1 & !is.na(impact.2yr$MPAID),])

# - Status & trend plot dataset, using matched data
Maya.2yr.status.trend <- Maya.BigFive.summary[1:4,]


# - Impact plot dataset
Maya.2yr.impacts <- Maya.BigFive.summary[5:6,c(1,3:12)]
...
Maya.BigFive <- rbind.data.frame(Maya.BigFive.t0,Maya.BigFive.t2)
...
Maya.sig.pos <- data.frame(mapply(i=Maya.2yr.impacts[Maya.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
                                  ...
                                  outcome.sd=c(sd(impact.2yr[impact.2yr$MPAID==1 & !is.na(impact.2yr$MPAID),22],na.rm=T),
                                               (sd(impact.2yr[impact.2yr$MPAID==1 & !is.na(impact.2yr$MPAID),36],na.rm=T)/100),
                                               sapply(impact.2yr[impact.2yr$MPAID==1 & !is.na(impact.2yr$MPAID),c(29,7,14)],sd,na.rm=T)),
