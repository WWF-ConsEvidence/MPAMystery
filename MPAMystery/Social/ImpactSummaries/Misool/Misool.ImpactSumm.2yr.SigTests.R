# ----
# code:  Misool 2Yr Impact Summary Statistical Tests
# git branch: MPAMystery --> Social --> ImpactSummaries --> Misool
# 
...
Mis.BigFive.MPAimpact.summ.st <- cbind.data.frame(rep.int(c("MPA","Control"),2),
                                                  c(rep("Baseline",2),rep("2 Year Post",2)),
                                                  rbind.data.frame(c(summarise(Mis.2yr.HHimpacts,
...
colnames(Mis.BigFive.MPAimpact.summ.st) <- c("Treatment","Year",colnames(Mis.2yr.impacts[,2:11]))
Mis.BigFive.MPAimpact.summ.st$Treatment <- factor(Mis.BigFive.MPAimpact.summ.st$Treatment,
                                                  levels=c("MPA","Control"),
                                                  ordered=T)
Mis.BigFive.MPAimpact.summ.st$Year <- factor(Mis.BigFive.MPAimpact.summ.st$Year,
                                             levels=c("Baseline","2 Year Post","4 Year Post"),
                                             ordered=T)

# --- Impact plot dataset
Mis.2yr.HHimpacts <- impact.2yr[impact.2yr$MPAID==6 & !is.na(impact.2yr$MPAID),]
Mis.2yr.impacts <- cbind.data.frame(c("MPA","Control"),
                                    rbind.data.frame(c(summarise(Mis.2yr.HHimpacts,
...
colnames(Mis.2yr.impacts) <- c("Treatment",colnames(Mis.2yr.impacts[2:11]))
Mis.2yr.impacts$Treatment <- factor(Mis.2yr.impacts$Treatment,
                                    levels=Mis.2yr.impacts$Treatment,
                                    ordered=T)

# --- Snapshot impact plots (all five indicators on one plot, standardized impacts)
Mis.2yr.std.impacts <- subset(std.impact.2yr[std.impact.2yr$MPA=="Misool Selatan Timur MPA",])
Mis.2yr.std.impacts$MPA <- factor(Mis.2yr.std.impacts$MPA,
                                  labels="Misool")

# 2.2 Dataset for statistical analysis
Mis.2yr.HHimpacts.1 <- Mis.2yr.HHimpacts[,c(2,5,9,12,16,19,23,26,30,33)]
colnames(Mis.2yr.HHimpacts.1) <- c("FS","FS","MA","MA","PA","PA","MT","MT","SE","SE")
Mis.BigFive <- cbind.data.frame(c(rep("Baseline",length(Mis.2yr.HHimpacts.1[,1])),
                                  rep("2 Year Post",length(Mis.2yr.HHimpacts.1[,1]))),
                                rbind.data.frame(c(Mis.2yr.HHimpacts.1[,c(1,3,5,7,9)]),
                                                 c(Mis.2yr.HHimpacts.1[,c(2,4,6,8,10)])))
colnames(Mis.BigFive) <- c("Year","FS","MA","PA","MT","SE")
...
Mis.wilcox.MPAimpact.summ.base.2yr <- as.data.frame(sapply(Mis.BigFive[,c(2:6)],
                                                           function(i){
                                                             wilcox.test(i~Year,
                                                                         data=Mis.BigFive,
                                                                         exact=F)
...
Mis.sig.labs <- data.frame(xaxis.st.baselabs=rep(paste("Baseline\nscore\n","(",
                           ...
                           snapshot.labs=mapply(i=p.vals.misool[order(p.vals.misool$Indicator),"p.val"],
                           ...
                           impact.labs=sapply(p.vals.misool[order(p.vals.misool$Indicator),"p.val"],
                                              function(i){
                                                c(ifelse(i<0.01,"*\n*\n*",
...
Mis.impact.arrows <- data.frame(mapply(i=Mis.2yr.impacts[Mis.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
                                       j=Mis.2yr.impacts[Mis.2yr.impacts$Treatment=="Control",c("PA","SE","MT","FS","MA")],
                                       function(i,j){
...
Mis.impact.arrows <- cbind.data.frame(Mis.impact.arrows,
                                      x=rep(0.6,4))

Mis.sig.pos <- data.frame(mapply(i=Mis.2yr.impacts[Mis.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
                                 j=Mis.2yr.impacts[Mis.2yr.impacts$Treatment=="Control",c("PA","SE","MT","FS","MA")],
                                 k=Mis.sig.labs$impact.labs,
                                 outcome.sd=c(sd(Mis.2yr.HHimpacts[,c(21)],na.rm=T),
                                              (sd(Mis.2yr.HHimpacts[,c(35)],na.rm=T)/100),
                                              sapply(Mis.2yr.HHimpacts[,c(28,6,13)],sd,na.rm=T)),
