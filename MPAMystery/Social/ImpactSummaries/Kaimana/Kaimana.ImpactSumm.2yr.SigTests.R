...
Kai.BigFive.MPAimpact.summ.st <- cbind.data.frame(rep.int(c("MPA","Control"),2),
                                                  c(rep("Baseline",2),rep("2 Year Post",2)),
                                                  rbind.data.frame(c(summarise(Kai.2yr.HHimpacts,
...
colnames(Kai.BigFive.MPAimpact.summ.st) <- c("Treatment","Year",colnames(Kai.2yr.impacts[,2:11]))
Kai.BigFive.MPAimpact.summ.st$Treatment <- factor(Kai.BigFive.MPAimpact.summ.st$Treatment,
                                                  levels=c("MPA","Control"),
                                                  ordered=T)
Kai.BigFive.MPAimpact.summ.st$Year <- factor(Kai.BigFive.MPAimpact.summ.st$Year,
                                             levels=c("Baseline","2 Year Post","4 Year Post"),
                                             ordered=T)

# --- Impact plot dataset

Kai.2yr.HHimpacts <- impact.2yr[impact.2yr$MPAID==3 & !is.na(impact.2yr$MPAID),]
Kai.2yr.impacts <- cbind.data.frame(c("MPA","Control"),
                                    rbind.data.frame(c(summarise(Kai.2yr.HHimpacts,
...
colnames(Kai.2yr.impacts) <- c("Treatment",colnames(Kai.2yr.impacts[2:11]))
Kai.2yr.impacts$Treatment <- factor(Kai.2yr.impacts$Treatment,
                                    levels=Kai.2yr.impacts$Treatment,
                                    ordered=T)

# --- Snapshot impact plots (all five indicators on one plot, standardized impacts)
Kai.2yr.std.impacts <- subset(std.impact.2yr[std.impact.2yr$MPA=="Kaimana MPA",])
Kai.2yr.std.impacts$MPA <- factor(Kai.2yr.std.impacts$MPA,
                                  labels="Kaimana")

# 2.2 Dataset for statistical analysis

Kai.2yr.HHimpacts.1 <- Kai.2yr.HHimpacts[,c(2,5,9,12,16,19,23,26,30,33)]
colnames(Kai.2yr.HHimpacts.1) <- c("FS","FS","MA","MA","PA","PA","MT","MT","SE","SE")
Kai.BigFive <- cbind.data.frame(c(rep("Baseline",length(Kai.2yr.HHimpacts.1[,1])),
                                  rep("2 Year Post",length(Kai.2yr.HHimpacts.1[,1]))),
                                rbind.data.frame(c(Kai.2yr.HHimpacts.1[,c(1,3,5,7,9)]),
                                                 c(Kai.2yr.HHimpacts.1[,c(2,4,6,8,10)])))
colnames(Kai.BigFive) <- c("Year","FS","MA","PA","MT","SE")
...
Kai.sig.labs <- data.frame(xaxis.st.baselabs=rep(paste("Baseline\nscore\n","(",
                           ...
                           snapshot.labs=mapply(i=p.vals.kaimana[order(p.vals.kaimana$Indicator),"p.val"],
                           ...
                           impact.labs=sapply(p.vals.kaimana[order(p.vals.kaimana$Indicator),"p.val"],
                                              function(i){
                                                c(ifelse(i<0.01,"*\n*\n*",
...
Kai.sig.pos <- data.frame(mapply(i=Kai.2yr.impacts[Kai.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
                                 ...
                                 outcome.sd=c(sd(Kai.2yr.HHimpacts[,c(21)],na.rm=T),
                                              (sd(Kai.2yr.HHimpacts[,c(35)],na.rm=T)/100),
                                              sapply(Kai.2yr.HHimpacts[,c(28,6,13)],sd,na.rm=T)),
