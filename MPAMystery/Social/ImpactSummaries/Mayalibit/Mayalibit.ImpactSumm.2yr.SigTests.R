# ----
# code:  Mayalibit 2Yr Impact Summary Statistical Tests
...
# --- Status & trend plots, using matched data
Maya.BigFive.MPAimpact.summ.st <- cbind.data.frame(rep.int(c("MPA","Control"),2),
                                                   c(rep("Baseline",2),rep("2 Year Post",2)),
                                                   rbind.data.frame(c(summarise(Maya.2yr.HHimpacts,
                                                                    ...
                                                                    c(summarise(Maya.2yr.HHimpacts,
...
colnames(Maya.BigFive.MPAimpact.summ.st) <- c("Treatment","Year",colnames(Maya.2yr.impacts[,2:11]))
Maya.BigFive.MPAimpact.summ.st$Treatment <- factor(Maya.BigFive.MPAimpact.summ.st$Treatment,
                                                   levels=c("MPA","Control"),
                                                   ordered=T)
Maya.BigFive.MPAimpact.summ.st$Year <- factor(Maya.BigFive.MPAimpact.summ.st$Year,
                                              levels=c("Baseline","2 Year Post","4 Year Post"),
                                              ordered=T)

# --- Impact plot dataset
Maya.2yr.HHimpacts <- impact.2yr[impact.2yr$MPAID==1 & !is.na(impact.2yr$MPAID),]
Maya.2yr.impacts <- cbind.data.frame(c("MPA","Control"),
                                     rbind.data.frame(c(summarise(Maya.2yr.HHimpacts,
...
colnames(Maya.2yr.impacts) <- c("Treatment",colnames(Maya.2yr.impacts[2:11]))
Maya.2yr.impacts$Treatment <- factor(Maya.2yr.impacts$Treatment,
                                     levels=Maya.2yr.impacts$Treatment,
                                     ordered=T)

# --- Snapshot impact plots (all five indicators on one plot, standardized impacts)
Maya.2yr.std.impacts <- subset(std.impact.2yr[std.impact.2yr$MPA=="Teluk Mayalibit MPA",])
Maya.2yr.std.impacts$MPA <- factor(Maya.2yr.std.impacts$MPA,
                                   labels="Mayalibit")

# 2.2 Dataset for statistical analysis

Maya.2yr.HHimpacts.1 <- Maya.2yr.HHimpacts[,c(2,5,9,12,16,19,23,26,30,33)]
colnames(Maya.2yr.HHimpacts.1) <- c("FS","FS","MA","MA","PA","PA","MT","MT","SE","SE")
Maya.BigFive <- cbind.data.frame(c(rep("Baseline",length(Maya.2yr.HHimpacts.1[,1])),
                                   rep("2 Year Post",length(Maya.2yr.HHimpacts.1[,1]))),
                                 rbind.data.frame(c(Maya.2yr.HHimpacts.1[,c(1,3,5,7,9)]),
                                                  c(Maya.2yr.HHimpacts.1[,c(2,4,6,8,10)])))
colnames(Maya.BigFive) <- c("Year","FS","MA","PA","MT","SE")
...
Maya.wilcox.MPAimpact.summ.base.2yr <- as.data.frame(sapply(Maya.BigFive[,c(2:6)],
                                                            function(i){
                                                              wilcox.test(i~Year,
                                                                          data=Maya.BigFive,
                                                                          exact=F)
...
Maya.sig.labs <- data.frame(xaxis.st.baselabs=rep(paste("Baseline\nscore\n","(",
                            ...
                            snapshot.labs=mapply(i=p.vals.mayalibit[order(p.vals.mayalibit$Indicator),"p.val"],
                            ...
                            impact.labs=sapply(p.vals.mayalibit[order(p.vals.mayalibit$Indicator),"p.val"],
                                               function(i){
                                                 c(ifelse(i<0.01,"*\n*\n*",
...
Maya.impact.arrows <- data.frame(mapply(i=Maya.2yr.impacts[Maya.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
...
Maya.impact.arrows <- cbind.data.frame(Maya.impact.arrows,
                                       x=rep(0.6,4))
...
Maya.sig.pos <- data.frame(mapply(i=Maya.2yr.impacts[Maya.2yr.impacts$Treatment=="MPA",c("PA","SE","MT","FS","MA")],
                                  ...
                                  outcome.sd=c(sd(Maya.2yr.HHimpacts[,c(21)],na.rm=T),
                                               (sd(Maya.2yr.HHimpacts[,c(35)],na.rm=T)/100),
                                               sapply(Maya.2yr.HHimpacts[,c(28,6,13)],sd,na.rm=T)),
