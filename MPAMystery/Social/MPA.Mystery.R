...
# KC_IndDemos_ForMPAMystery and HH_tbl_SETTLEMENT -- in BHS Master Database in Access
...
# 1.1 Call libraries and establish ODBC connection to access database
library(plyr)
library(dplyr)
library(ggplot2)
library(varhandle)
...
# 1.2 Import data
HHData <- sqlFetch(BHS.MPAMysteryDB,"KC_HHData_ForMPAMystery")
...
IndDemos <- sqlFetch(BHS.MPAMysteryDB,"KC_IndDemos_ForMPAMystery")
...
HeadOfHH <- IndDemos[IndDemos$RelationHHH==0,1:4]

Settlements <- sqlFetch(BHS.MPAMysteryDB,"HH_tbl_SETTLEMENT")
Settlements <- Settlements[-c(83,95:100),c(1,3:5)]
...
# --- Define Monitoring Year for each MPA
...
HHData$MonitoringYear <- mapply(a=HHData$HouseholdID,
                                ...
                                function(a,b){
                                  ifelse(b==MonitoringYear$Baseline[MonitoringYear$HouseholdID==a],"Baseline",
                                         ifelse(b==MonitoringYear$TwoYear[MonitoringYear$HouseholdID==a],"2 Year Post",
                                                ifelse(b==MonitoringYear$FourYear[MonitoringYear$HouseholdID==a],"4 Year Post",NA)))
                                })

# --- Subset variables from HHData for Big Five calculations & descriptive statistics
FS <- HHData[,c(1:4,11:17)]
MA <- HHData[,c(1:3,29:40)]
PA <- HHData[,c(1:3,47:53)]
MT <- HHData[,c(1:3,59:64)]
HHLivelihood <- HHData[,c(1:3,79:90)]
HHDemos <- HHData[,c(1:3,91:95)]

# --- Source Revised_script to upload 2 year impact data and p values for impacts
setwd('C:/Users/claborn-intern/Dropbox (MPAMystery)/GitHub/WWF-conservation-evidence/')
source('MPAMystery/Social/Revised_script_KC.R',print.eval=T)
...
# 
# std.impact.2yr <- read.csv('Social impacts, BHS -- Kelly/R codes & data/MPA_2yr_std_impacts&se.csv',header=TRUE)
# std.impact.2yr <- cbind.data.frame(std.impact.2yr[,1:4])
...
#                                           ordered=T)
# 
# seascape.impact.2yr <- read.csv('Social impacts, BHS -- Kelly/R codes & data/BHS_2yr_std_impacts.csv',header=TRUE)
...
setwd('C:/Users/claborn-intern/Dropbox/GitHub/WWF-conservation-evidence/MPAMystery/Social')
...
FS$FSIndex <- ifelse(FS$'RemoveFS?'=="No",
                     rowSums(FS[,6:11],
                             na.rm=TRUE),
                     NA)
FS$FSIndex <- as.numeric(ifelse(FS$FSIndex=="0","6.06",
                                ifelse(FS$FSIndex=="1","5.38",
                                       ifelse(FS$FSIndex=="2","4.5",
                                              ifelse(FS$FSIndex=="3","3.77",
                                                     ifelse(FS$FSIndex=="4","2.99",
                                                            ifelse(FS$FSIndex=="5","2.04","0")))))))
...
BigFive.allvar$SettlementID <- BigFive.allvar$SettlementID.x
BigFive.allvar <- left_join(BigFive.allvar,Settlements,by="SettlementID")
BigFive.allvar <- left_join(BigFive.allvar,HHData[,c("HouseholdID","MonitoringYear")],by="HouseholdID")

# 2.2 "Big Five" Indexes Data Frame, all MPAs, all years
...
# 2.3 "Big Five" data, averaged by settlement for each monitoring year
BigFive.SettleGroup <- 
  BigFive %>%
  ...
  summarise(FSMean=round(mean(FSIndex,na.rm=T),2),
            ...
            PAMean=round(mean(PAIndex,na.rm=T)/sqrt(length(PAIndex)),2),
...
BigFive.SettleGroup <- left_join(BigFive.SettleGroup,
                                 Settlements,
                                 by="SettlementID")

# 2.4 "Big Five" data, averaged by control group (per MPA) for each monitoring year
...
BigFive.ControlGroup <- cbind.data.frame(rep.int(NA,6),BigFive.ControlGroup[,c(2:12,1)],rep.int("Control",6),rep.int(0,6))
colnames(BigFive.ControlGroup) <- colnames(BigFive.SettleGroup)

# 2.5 "Big Five" data, averaged by MPA for each monitoring year
BigFive.MPAGroup <- 
...
BigFive.MPAGroup <- cbind.data.frame(rep.int(NA,6),BigFive.MPAGroup[,c(2:12,1)],rep.int("MPA",6),rep.int(0,6))
colnames(BigFive.MPAGroup) <- colnames(BigFive.SettleGroup)

#   2.6 For status plots
BigFive.AllGroup <- rbind.data.frame(BigFive.SettleGroup,
                                     BigFive.ControlGroup,
                                     BigFive.MPAGroup)

# 2.7 For trend plots
BigFive.YearGroup <- BigFive.AllGroup[,c(2:13,1,14:15)]
colnames(BigFive.YearGroup) <- c("Year",colnames(BigFive.YearGroup[2:15]))
BigFive.YearGroup$Year <- factor(BigFive.YearGroup$Year,
                                 levels=c("Baseline","2 Year Post","4 Year Post"),
                                 ordered=T)
BigFive.YearGroup <- BigFive.YearGroup[order(BigFive.YearGroup$Year),]
BigFive.YearGroup$Year.treat <- factor(mapply(i=BigFive.YearGroup$Year,
                                              j=BigFive.YearGroup$Treatment,
                                              function(i,j){
                                                ifelse(i=="4 Year Post" & j==0, "4yr.cont",
                                                       ifelse(i=="4 Year Post" & j==1, "4yr.treat",
                                                              ifelse(i=="2 Year Post" & j==0, "2yr.cont",
                                                                     ifelse(i=="2 Year Post" & j==1,"2yr.treat",
                                                                            ifelse(i=="Baseline" & j==0, "Base.cont","Base.treat")))))}),
                                       levels=c("4yr.cont","2yr.cont","Base.cont",
                                                "4yr.treat","2yr.treat","Base.treat"),
                                       ordered=TRUE)
BigFive.YearGroup$Year.treat.2 <- factor(mapply(i=BigFive.YearGroup$Year,
                                                j=BigFive.YearGroup$Treatment,
                                                function(i,j){
                                                  ifelse(i=="4 Year Post" & j==0, "4yr.cont",
                                                         ifelse(i=="4 Year Post" & j==1, "4yr.treat",
...
# --- Plot formatting, fill colors and dummy rows
dummyvar <- matrix(c(0,0,0),nrow=3,ncol=31)

dummyrow.allgroup <- cbind.data.frame(dummyvar,NA,"",0)
colnames(dummyrow.allgroup) <- colnames(BigFive.AllGroup)

dummyrow.yeargroup <- cbind.data.frame(c("Baseline","2 Year Post","4 Year Post"),
                                       dummyvar[,1:11],NA,"",0,c("Base.cont","2yr.cont","4yr.cont"),
                                       c("Base.dummy","2yr.dummy","4yr.dummy"))
colnames(dummyrow.yeargroup) <- colnames(BigFive.YearGroup)

BigFive.AllGroup <- rbind.data.frame(BigFive.AllGroup[1:107,],
                                     dummyrow.allgroup[1,1:34],
                                     BigFive.AllGroup[108:119,])
BigFive.AllGroup$SettlementName <- factor(BigFive.AllGroup$SettlementName,
                                          levels=c("MPA","Control","",
                                                   as.character(BigFive.AllGroup$SettlementName)),
                                          ordered=TRUE)


BigFive.YearGroup <- rbind.data.frame(BigFive.YearGroup[1:107,],
                                      dummyrow.yeargroup[1,],
                                      BigFive.YearGroup[108:226,],
                                      dummyrow.yeargroup[2,],
                                      BigFive.YearGroup[227:345,],
                                      dummyrow.yeargroup[3,],
                                      BigFive.YearGroup[346:357,])
BigFive.YearGroup$SettlementName <- factor(BigFive.YearGroup$SettlementName,
                                           levels=c("MPA","Control","",
                                                    as.character(BigFive.YearGroup$SettlementName[1:107])),
                                           ordered=TRUE)
BigFive.YearGroup$Year.treat.2 <- factor(BigFive.YearGroup$Year.treat.2,
                                         levels=c("4yr.cont","2yr.cont","Base.cont",
                                                  "4yr.dummy","2yr.dummy","Base.dummy",
                                                  "4yr.treat","2yr.treat","Base.treat"),
                                         ordered=TRUE)
...
############################################################
# ----
setwd("C:/Users/claborn-intern/Dropbox/DATA_ANALYSIS/")
# 3.1 Call data and wrangle dataframe
HHEthnicity <- read.delim ("2_PROJECTS/1_COMPUTE_HWB_INDICATORS/Raw_data/Ethnic_2015_0705.txt")
...
HHDemos.context.1$InterviewYear <- factor(HHDemos.context.1$InterviewYear,
                                        levels=c("2010","2011","2012","2013","2014","2015","2016"),
                                        ordered=T)
...
# 3.2 Settlement-level analysis, for status plots

Techreport.status <- group_by(CurrentDemos.context,SettlementID)
Techreport.status <- summarise(Techreport.status,
                               MPAID=unique(MPAID),
...
# --- Current year
Techreport.status.MPA <- group_by(CurrentDemos.context,MPAID)
Techreport.status.MPA <- summarise(Techreport.status.MPA,
                                   HHH.female=(length(HeadofHH.gender[HeadofHH.gender==0 &
                                                                        !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
                                   HHH.male=(length(HeadofHH.gender[HeadofHH.gender==1 &
                                                                      !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
...
# --- Baseline year
Techreport.trend.Baseline.MPA <- group_by(HHDemos.context[HHDemos.context$MonitoringYear=="Baseline",],MPAID)
Techreport.trend.Baseline.MPA <- summarise(Techreport.trend.Baseline.MPA,
                                           HHH.female=(length(HeadofHH.gender[HeadofHH.gender==0 &
                                                                                !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
                                           HHH.male=(length(HeadofHH.gender[HeadofHH.gender==1 &
                                                                              !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
...
# --- 2 year cross-section
Techreport.trend.2yr.MPA <- group_by(HHDemos.context[HHDemos.context$MonitoringYear=="2 Year Post",],MPAID)
Techreport.trend.2yr.MPA <- summarise(Techreport.trend.2yr.MPA,
                                      HHH.female=(length(HeadofHH.gender[HeadofHH.gender==0 &
                                                                           !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
                                      HHH.male=(length(HeadofHH.gender[HeadofHH.gender==1 &
                                                                         !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
...
#  3.4 Settlement-level analysis, for annex plots (use Techreport.status (above) for most current cross section)

# --- Baseline year
Techreport.trend.Baseline.Sett <- group_by(HHDemos.context[HHDemos.context$MonitoringYear=="Baseline",],SettlementID)
Techreport.trend.Baseline.Sett <- summarise(Techreport.trend.Baseline.Sett,
                                            MPAID=unique(MPAID),
                                            SettlementName=unique(SettlementName),
                                           HHH.female=(length(HeadofHH.gender[HeadofHH.gender==0 &
                                                                                !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
...
Techreport.trend.Baseline.Sett <- rbind.data.frame(Techreport.trend.Baseline.Sett,
                                                   data.frame(SettlementID=c(72,104:112,113:115),MPAID=c(5,rep(2,9),rep(3,3)),
                                                              SettlementName=c(as.character(BigFive.SettleGroup$SettlementName[BigFive.SettleGroup$SettlementID==72]),
                                                                               as.character(BigFive.SettleGroup$SettlementName[BigFive.SettleGroup$MPAID==2 &
                                                                                                                                 BigFive.SettleGroup$Treatment==1 &
                                                                                                                                 BigFive.SettleGroup$SettlementID>100]),
                                                                               as.character(BigFive.SettleGroup$SettlementName[BigFive.SettleGroup$MPAID==3 &
                                                                                                                    BigFive.SettleGroup$SettlementID>110])),
                                                              as.data.frame(matrix(rep(NA,37),ncol=37,nrow=13,
                                                                     dimnames=list(NULL,colnames(Techreport.trend.Baseline.Sett[4:40]))))))

# --- 2 year cross-section
Techreport.trend.2yr.Sett <- group_by(HHDemos.context[HHDemos.context$MonitoringYear=="2 Year Post",],SettlementID)
Techreport.trend.2yr.Sett <- summarise(Techreport.trend.2yr.Sett,
                                       MPAID=unique(MPAID),
                                       SettlementName=unique(SettlementName),
                                      HHH.female=(length(HeadofHH.gender[HeadofHH.gender==0 &
                                                                           !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
...
MPAimpact.intro.context$Settlement.LeastFishers <- sapply(MPAimpact.intro.context$MPAID,
                                                          function(i){
                                                            min <- tail(sort(Techreport.status$Percent.PrimaryOcc.Fish[Techreport.status$MPAID==i],decreasing=T),1)
                                                            paste(as.character(Techreport.status$SettlementName[Techreport.status$Percent.PrimaryOcc.Fish==min &
                                                                                                            Techreport.status$MPAID==i]),paste(round(min,2),"%",sep=""),sep=", ")
                                                          })
MPAimpact.intro.context$Settlement.MostFishers <- sapply(MPAimpact.intro.context$MPAID,
                                                         function(i){
                                                           max <- head(sort(Techreport.status$Percent.PrimaryOcc.Fish[Techreport.status$MPAID==i],decreasing=T),1)
                                                           paste(as.character(Techreport.status$SettlementName[Techreport.status$Percent.PrimaryOcc.Fish==max &
                                                                                                           Techreport.status$MPAID==i]),paste(round(max,2),"%",sep=""),sep=", ")
                                                         })
...
Days.unwell <- summarise(Days.unwell,
                         DaysUnwell=sum(DaysUnwellClean,na.rm=T))
...
Days.unwell <- left_join(Days.unwell,
                         MPA.currentyear,
                         by="MPAID")
Days.unwell.status <- Days.unwell[Days.unwell$InterviewYear==Days.unwell$CurrentYear &
                                    Days.unwell$Treatment==1,]
Days.unwell.2yr <- Days.unwell[Days.unwell$MonitoringYear=="2 Year Post" &
                                 Days.unwell$Treatment==1,]
Days.unwell.baseline <- Days.unwell[Days.unwell$MonitoringYear=="Baseline" &
                                      Days.unwell$Treatment==1,]

# ---Days unwell, by MPA ("Days.unwell.ByMPA" is most current cross-section of data)
Days.unwell.ByMPA <- group_by(Days.unwell.status,MPAID)
Days.unwell.ByMPA <- summarise(Days.unwell.ByMPA,
                               Days.unwell=mean(DaysUnwell,na.rm=T),
                               Days.unwell.err=sd(DaysUnwell,na.rm=T)/sqrt(length(DaysUnwell)))

Days.unwell.baseline.ByMPA <- group_by(Days.unwell.baseline,MPAID)
Days.unwell.baseline.ByMPA <- summarise(Days.unwell.baseline.ByMPA,
                                        Days.unwell=mean(DaysUnwell,na.rm=T),
                                        Days.unwell.err=sd(DaysUnwell,na.rm=T)/sqrt(length(DaysUnwell)))

Days.unwell.2yr.ByMPA <- group_by(Days.unwell.2yr,MPAID)
Days.unwell.2yr.ByMPA <- summarise(Days.unwell.2yr.ByMPA,
                                   Days.unwell=mean(DaysUnwell,na.rm=T),
                                   Days.unwell.err=sd(DaysUnwell,na.rm=T)/sqrt(length(DaysUnwell)))

# ---Days unwell, by settlement ("Days.unwell.BySett" is most current cross-section of data)
Days.unwell.BySett <- group_by(Days.unwell.status,SettlementID)
Days.unwell.BySett <- summarise(Days.unwell.BySett,
                                MPAID=unique(MPAID),
                                Days.unwell=mean(DaysUnwell,na.rm=T),
                                Days.unwell.err=sd(DaysUnwell,na.rm=T)/sqrt(length(DaysUnwell)))

Days.unwell.2yr.BySett <- group_by(Days.unwell.2yr,SettlementID)
Days.unwell.2yr.BySett <- summarise(Days.unwell.2yr.BySett,
                                    MPAID=unique(MPAID),
                                    Days.unwell=mean(DaysUnwell,na.rm=T),
                                    Days.unwell.err=sd(DaysUnwell,na.rm=T)/sqrt(length(DaysUnwell)))

Days.unwell.baseline.BySett <- group_by(Days.unwell.baseline,SettlementID)
Days.unwell.baseline.BySett <- summarise(Days.unwell.baseline.BySett,
                                         MPAID=unique(MPAID),
                                         Days.unwell=mean(DaysUnwell,na.rm=T),
                                         Days.unwell.err=sd(DaysUnwell,na.rm=T)/sqrt(length(DaysUnwell)))

Days.unwell.control <- Days.unwell[Days.unwell$InterviewYear==Days.unwell$CurrentYear &
                                     Days.unwell$Treatment==0,]
Days.unwell.control <- group_by(Days.unwell.control,MPAID)
Days.unwell.control <- summarise(Days.unwell.control,
                                 Days.unwell=mean(DaysUnwell,na.rm=T))

# --- COMPARE MPAS TO CONTROLS 
Techreport.status.control <- group_by(CurrentDemos.control,MPAID)
Techreport.status.control <- summarise(Techreport.status.control,
                                       HHH.female=(length(HeadofHH.gender[HeadofHH.gender==0 &
                                                                            !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
                                       HHH.male=(length(HeadofHH.gender[HeadofHH.gender==1 &
                                                                          !is.na(HeadofHH.gender)])/length(HeadofHH.gender[!is.na(HeadofHH.gender)]))*100,
                                       ...
                                       Child.FS.no=(length(Child.FS.category[Child.FS.category=="No or insufficient evidence" & !is.na(Child.FS.category)])/length(Child.FS.category[!is.na(Child.FS.category)]))*100,
                                       Child.FS.yes=(length(Child.FS.category[Child.FS.category=="Evidence" & !is.na(Child.FS.category)])/length(Child.FS.category[!is.na(Child.FS.category)]))*100)
Compare.prop.data <- rbind.data.frame(data.frame(Treatment="MPA",Techreport.status.MPA[,-4]),
                                      data.frame(Treatment="Control",Techreport.status.control[,-4]))
...
Compare.cont.data <- rbind.data.frame(data.frame(Treatment="MPA",BigFive.MPAGroup[,c(31,5,11,17,23,29)],
                                                 Techreport.status.MPA[,4],Days.unwell.ByMPA[,2]),
                                      data.frame(Treatment="Control",BigFive.ControlGroup[,c(31,5,11,17,23,29)],
                                                 Techreport.status.control[,4],Days.unwell.control[,2]))
...
AgeGenderDemos.base <- AgeGenderDemos[AgeGenderDemos$MonitoringYear=="Baseline",]
AgeGenderDemos.2yr <- AgeGenderDemos[AgeGenderDemos$MonitoringYear=="2 Year Post",]
AgeGenderDemos.status <- AgeGenderDemos[AgeGenderDemos$InterviewYear==MPA.currentyear$CurrentYear,]

AgeGenderDemos.status.ByMPA <- group_by(AgeGenderDemos.status,MPAID)
AgeGenderDemos.status.ByMPA <- summarise(AgeGenderDemos.status.ByMPA,
                                         Male.0.4=(length(IndividualAgeClean[IndividualAgeClean<=4 &
...
AgeGenderDemos.base.ByMPA <- group_by(AgeGenderDemos.base,MPAID)
AgeGenderDemos.base.ByMPA <- summarise(AgeGenderDemos.base.ByMPA,
                                         Male.0.4=(length(IndividualAgeClean[IndividualAgeClean<=4 &
                                                                               !is.na(IndividualAgeClean) &
                                                                               IndividualGenderClean==1&
                                                                               !is.na(IndividualGenderClean)])/
...
AgeGenderDemos.2yr.ByMPA <- group_by(AgeGenderDemos.2yr,MPAID)
AgeGenderDemos.2yr.ByMPA <- summarise(AgeGenderDemos.2yr.ByMPA,
                                         Male.0.4=(length(IndividualAgeClean[IndividualAgeClean<=4 &
                                                                               !is.na(IndividualAgeClean) &
                                                                               IndividualGenderClean==1&
                                                                               !is.na(IndividualGenderClean)])/
...
############################################################
#
#SECTION 5: Define global themes and environments
#
############################################################
# ----
...
# 5.4 MPA Impact Summary "Big Five" plot labels
...
fillcols.status <- factor(ifelse(BigFive.AllGroup$Treatment==1,"#65A164","#505050"))
errcols.status <- ifelse(BigFive.AllGroup$Treatment==1,
                         "#3E623E",ifelse(BigFive.AllGroup$MPAID==0,
                                          "#FFFFFF","#202020"))

fillcols.trend <- factor(ifelse(BigFive.YearGroup$Year.treat=="Base.treat" |
                                  BigFive.YearGroup$Year.treat=="2yr.treat" |
                                  BigFive.YearGroup$Year.treat=="4yr.treat","#65A164",
                                "#505050"),
                         levels=c(rep("#505050",3),rep("#65A164",3)),
                         ordered=TRUE)
errcols.trend <- factor(ifelse(BigFive.YearGroup$Year.treat.2=="4yr.treat" |
                                 BigFive.YearGroup$Year.treat.2=="2yr.treat" |
                                 BigFive.YearGroup$Year.treat.2=="Base.treat","#3E623E",
                               ifelse(BigFive.YearGroup$Year.treat.2=="4yr.dummy" |
                                        BigFive.YearGroup$Year.treat.2=="2yr.dummy" |
                                        BigFive.YearGroup$Year.treat.2=="Base.dummy","#FFFFFF","#202020")),
                        levels=c(rep("#202020",3),rep("#FFFFFF",3),rep("#3E623E",3)),
                        ordered=TRUE)
