...
Kai.TechReport.MPAHouseholdData <- left_join(MPA.TechReport.SigTest.Data[MPA.TechReport.SigTest.Data$Treatment==1 &
                                                                            MPA.TechReport.SigTest.Data$MonitoringYear== "4 Year Post" &
                                                                            MPA.TechReport.SigTest.Data$MPAID==3,], 
                                              Days.unwell.status[Days.unwell.status$MPAID==3,c("HouseholdID","DaysUnwell")],
                                              by="HouseholdID")
Kai.TechReport.MPAHouseholdData$SettlementName <- factor(Kai.TechReport.MPAHouseholdData$SettlementName)

### "MPA versus Control" dataset
Kai.TechReport.MPAvControl <- left_join(MPA.TechReport.SigTest.Data[MPA.TechReport.SigTest.Data$MPAID==3 &
                                                                       MPA.TechReport.SigTest.Data$MonitoringYear=="4 Year Post",],
                                         Days.unwell[Days.unwell$MPAID==3 &
                                                       Days.unwell$MonitoringYear=="4 Year Post",
                                                     c("HouseholdID","DaysUnwell")],
                                         by="HouseholdID")
Kai.TechReport.MPAvControl$MPA.v.Control <- factor(ifelse(Kai.TechReport.MPAvControl$Treatment==1,"MPA","Control"))

### "Settlement Means" dataset
Kai.TechReport.SettlementMeans <- left_join(BigFive.YearGroup[BigFive.YearGroup$Treatment==1 &
                                                                 BigFive.YearGroup$Year=="4 Year Post" &
                                                                 BigFive.YearGroup$MPAID==3,
                                                               c(2,4,6,8,10,13)],
                                             Techreport.status[Techreport.status$MPAID==3,
                                                               c(1,3,6)],  
                                             by="SettlementID")
Kai.TechReport.SettlementMeans <- left_join(Kai.TechReport.SettlementMeans,
                                             Days.unwell.BySett[Days.unwell.BySett$MPAID==3,c(1,3,2)],
                                             by="SettlementID")

Kai.TechReport.SettlementMeans <- Kai.TechReport.SettlementMeans[!is.na(Kai.TechReport.SettlementMeans$SettlementName),]
colnames(Kai.TechReport.SettlementMeans) <- c("FSIndex","MAIndex","PAIndex","MTIndex","SERate",
                                               "SettlementID","SettlementName","TimeMarketClean","DaysUnwell","MPAID")

### "Trend" dataset
Kai.Trend.Data <- left_join(MPA.TechReport.SigTest.Data[MPA.TechReport.SigTest.Data$Treatment==1 &
                                                           MPA.TechReport.SigTest.Data$MPAID==3,],
                             Days.unwell[Days.unwell$MPAID==3 &
...
even.number.setts.function.Kai <- mapply(a=Kai.TechReport.SettlementMeans[,c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarketClean","DaysUnwell")],
...
median.setts.Kai <- mapply(i=Kai.TechReport.SettlementMeans[,c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarketClean","DaysUnwell")],
...
sigvals.Sett.Kai <- cbind.data.frame(non.parametric.test.settlements.Kai[order(non.parametric.test.settlements.Kai$"FSIndex.SettlementName"),
...
sigvals.Sett.Kai <- sigvals.Sett.Kai[,c(1,2,4,6,8,10,12,14)]
...
non.parametric.test.MPAvControl.Kai <- data.frame(mapply(a=c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarketClean","DaysUnwell"),
                                                          function(a){
...
sigvals.MPA.Kai <- cbind.data.frame("MPA",non.parametric.test.MPAvControl.Kai)
colnames(sigvals.MPA.Kai) <- colnames(sigvals.Sett.Kai)

null.row.sigvals.Kai <- matrix(rep(NA,length(sigvals.MPA.Kai)),ncol=length(sigvals.MPA.Kai),dimnames=list(NULL,colnames(sigvals.MPA.Kai)))

sigvals.Kai <- rbind.data.frame(sigvals.MPA.Kai,
...
trend.non.parametric.test.byMPA.Kai <- data.frame(mapply(i=Kai.Trend.Data[,c("FSIndex","MAIndex","PAIndex","MTIndex","SERate","TimeMarketClean","DaysUnwell")],
                                                          function(i){
...
trend.sigvals.Kai <- cbind.data.frame(MonitoringYear="p.value",trend.non.parametric.test.byMPA.Kai["sl",1],NA,trend.non.parametric.test.byMPA.Kai["sl",2],
...
annex.sigvals.Kai <- rbind.data.frame(cbind.data.frame(SettlementName="MPA",trend.non.parametric.test.byMPA.Kai["sl",]),
                                       null.row.sigvals.Kai,
